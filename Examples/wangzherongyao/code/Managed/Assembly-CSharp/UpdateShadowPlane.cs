...
{
    [CompilerGenerated]
    private static UnityExtension.FindChildDelegate <>f__am$cache4;
    [CompilerGenerated]
    ...
    public void CalcHeight(SkinnedMeshRenderer renderer, ref bool posUpdated, ref float meshHeight, float invScale)
    {
        ...
        if ((bones != null) && (bones.Length != 0))
        {
            ...
            {
                b = Mathf.Max(bones[i].position.y - y, b);
            }
            meshHeight = Mathf.Max(b, meshHeight);
            meshHeightMap.Add(renderer.sharedMesh, b * invScale);
        }
    }
    ...
    private void EnablePlaneShadow()
    {
        if (this.shadowMesh == null)
        {
            ...
            {
                <>f__am$cache4 = delegate (GameObject obj) {
                    if (!(obj.name == "Shadow") && (obj.name.IndexOf("Shadow_") != 0))
                    {
                        return false;
                    }
            ...
            this.shadowMesh = base.gameObject.FindChildBFS(<>f__am$cache4);
            if (this.shadowMesh == null)
            {
    ...
    private bool SetIdlePos()
    {
        ...
        AnimationClip animation = componentInChildren.GetClip("Idle");
        if (animation == null)
        {
            return false;
        }
        base.gameObject.SampleAnimation(animation, 0f);
        return true;
    }

    public void Update()
    {
        if ((this.shadowMesh != null) && this.shadowMesh.activeSelf)
        {
            Transform transform = this.shadowMesh.transform;
            Vector3 position = transform.position;
            position.y = base.actor.groundY.scalar;
        ...
        if ((this.mats != null) && (this.mats.Count > 0))
        {
            ...
            vector2 = new Vector4(0f, 1f, 0f, 0f) {
                w = base.actor.groundY.scalar,
                w = vector2.w + 0.05f
            };
            float num = Mathf.Abs((float) (PlaneShadowSettings.shadowProjDir.y / Mathf.Max(0.001f, this.height)));
            for (int i = 0; i < this.mats.Count; i++)
            {
                Material material = this.mats[i];
                material.SetVector("_ShadowPlane", vector2);
                material.SetFloat("_ShadowInvLen", num);
