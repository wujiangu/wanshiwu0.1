namespace Assets.Scripts.GameSystem
{
    ...
    public class IDIPSys : MonoSingleton<IDIPSys>
    {
        ...
        {
            return new <DownloadImage>c__Iterator1A { preUrl = preUrl, callBack = callBack, <$>preUrl = preUrl, <$>callBack = callBack, <>f__this = this };
        }

        [DebuggerHidden]
        public IEnumerator DownloadImageByTag(string preUrl, int ImageIDx, LoadRCallBack2 callBack, string tagPath)
        {
            return new <DownloadImageByTag>c__Iterator1B { preUrl = preUrl, tagPath = tagPath, callBack = callBack, ImageIDx = ImageIDx, <$>preUrl = preUrl, <$>tagPath = tagPath, <$>callBack = callBack, <$>ImageIDx = ImageIDx, <>f__this = this };
        }
        ...
        {
            return new <GetChannelID>c__Iterator19 { <>f__this = this };
        }
        ...
        private void ProcessGetNoticeList(CSPkg msg)
        {
            ...
            if ((this.m_IDIPDataList.Count > 0) && Singleton<LobbyUISys>.GetInstance().IsInLobbyForm())
            {
                this.ShowForm();
            }
        }
        ...
        private void SelectMenuItem(int index)
        {
            ...
            for (int i = 0; i < this.m_IDIPItemList.Count; i++)
            {
                if (i == index)
                {
                    ...
                    item.glow.gameObject.CustomSetActive(true);
                }
                else
                {
                    IDIPItem item2 = this.m_IDIPItemList[i];
                    item2.glow.gameObject.CustomSetActive(false);
                }
            }
            if ((index >= 0) && (index < this.m_IDIPDataList.Count))
            {
                ...
                if (curData != null)
                {
                    ...
                    IDIPItem item3 = this.m_IDIPItemList[index];
                    item3.RedSpot.SetActive(false);
                    if (curData != null)
                    {
                        if (curData.bLoad)
                        {
        ...
        {
            PlayerPrefs.SetInt(string.Format("Notice|{0}|{1}", this.m_MyOpenID, noticeTime.ToString()), 1);
        }
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                if ((data != null) && (i < num3))
                {
                    if (!data.bVisited)
                    {
                        num++;
                        IDIPItem item = this.m_IDIPItemList[i];
                        item.RedSpot.CustomSetActive(true);
                    }
                    else
                    {
                        IDIPItem item2 = this.m_IDIPItemList[i];
                        item2.RedSpot.CustomSetActive(false);
                    }
                }
            }
            this.m_nRedPoint = num;
        ...
        [CompilerGenerated]
        private sealed class <DownloadImage>c__Iterator1A : IDisposable, IEnumerator, IEnumerator<object>
        {
            ...
            public bool MoveNext()
            {
                ...
                switch (num)
                {
                    ...
                    case 1:
                        ...
                        {
                            Debug.Log("loadimageerror " + this.<www>__4.error);
                            break;
                        }
                        this.<tex>__5 = this.<www>__4.texture;
                        if ((this.<tex>__5 != null) && (this.<localCachePath>__1 != null))
                        {
                            File.WriteAllBytes(this.<localCachePath>__1, this.<www>__4.bytes);
                        }
                        if (this.callBack != null)
                        {
                            this.callBack(this.<tex>__5);
                        }
                        break;
        ...
        [CompilerGenerated]
        private sealed class <DownloadImageByTag>c__Iterator1B : IDisposable, IEnumerator, IEnumerator<object>
        {
            internal object $current;
            internal int $PC;
            ...
            public bool MoveNext()
            {
                ...
                switch (num)
                {
                    ...
                    case 1:
                        if (!string.IsNullOrEmpty(this.<www>__4.error))
                        {
                            Debug.Log("DownloadImageByTag " + this.<www>__4.error);
                            break;
                        }
                        this.<tex>__5 = this.<www>__4.texture;
                        if ((this.<tex>__5 != null) && (this.<localCachePath>__1 != null))
                        ...
                        if (this.callBack != null)
                        {
                            this.callBack(this.<tex>__5, this.ImageIDx);
                        }
                        break;
        ...
        [CompilerGenerated]
        private sealed class <GetChannelID>c__Iterator19 : IDisposable, IEnumerator, IEnumerator<object>
        {
            internal object $current;
            internal int $PC;
            internal IDIPSys <>f__this;
            ...
            public bool MoveNext()
            {
                ...
                switch (num)
                {
                    ...
                    case 1:
                        ...
                        {
                            Debug.Log("GetChannelID3 = " + this.<www>__1.error);
                            break;
                        }
