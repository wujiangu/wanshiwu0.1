namespace Assets.Scripts.GameSystem
{
    ...
    internal class CAdventureSys : Singleton<CAdventureSys>
    {
        ...
        {
            ResLevelCfgInfo dataByKey = GameDataMgr.levelDatabin.GetDataByKey(this.currentLevelId);
        ...
        private bool CheckTeamNum()
        {
            ...
            COMDT_BATTLELIST_LIST defaultBattleListInfo = Singleton<CHeroSelectSystem>.GetInstance().m_defaultBattleListInfo;
            int num = 0;
            if ((defaultBattleListInfo == null) || (defaultBattleListInfo.dwListNum == 0))
            {
                return false;
            }
            for (int i = 0; i < defaultBattleListInfo.dwListNum; i++)
            {
                if (defaultBattleListInfo.astBattleList[i].dwBattleListID == dataByKey.dwBattleListID)
                {
                    if (defaultBattleListInfo.astBattleList[i].stBattleList.wHeroCnt == 0)
                    {
                        return false;
                    }
                    for (int j = 0; j < defaultBattleListInfo.astBattleList[i].stBattleList.wHeroCnt; j++)
                    {
                        if (defaultBattleListInfo.astBattleList[i].stBattleList.BattleHeroList[j] > 0)
                        {
                            num++;
                        }
                    }
        ...
        {
            CHeroSelectSystem.SendSingleGameStartMsgSkipHeroSelect(this.currentLevelId, this.currentDifficulty);
        }
        ...
        {
            <GetLevelCfg>c__AnonStorey4D storeyd = new <GetLevelCfg>c__AnonStorey4D {
                ChapterId = ChapterId,
                Seq = Seq
            };
            return GameDataMgr.levelDatabin.FindIf(new Func<ResLevelCfgInfo, bool>(storeyd, (IntPtr) this.<>m__54));
        }

        public static int GetLevelId(int ChapterId, int Seq, int Difficulty)
        {
            <GetLevelId>c__AnonStorey4E storeye = new <GetLevelId>c__AnonStorey4E {
                ChapterId = ChapterId,
                Seq = Seq,
                Difficulty = Difficulty
            };
            ResLevelCfgInfo info = GameDataMgr.levelDatabin.FindIf(new Func<ResLevelCfgInfo, bool>(storeye, (IntPtr) this.<>m__55));
        ...
        public override void Init()
        {
            ...
            Singleton<EventRouter>.instance.AddEventHandler(EventID.SINGLEGAME_ERR_FREEHERO, new Action(this, (IntPtr) this.OnFreeHeroChanged));
            Singleton<EventRouter>.instance.AddEventHandler(EventID.GLOBAL_REFRESH_TIME, new Action(null, (IntPtr) ResetElitePlayNum));
        }
        ...
        {
            ResLevelCfgInfo dataByKey = GameDataMgr.levelDatabin.GetDataByKey(inLevelId);
            ...
            else
            {
                ...
                if (Singleton<CRoleInfoManager>.GetInstance().GetMasterRoleInfo() != null)
                {
                    ...
                    if (info2 != null)
                    {
                        ...
                        Singleton<CTopLobbyEntry>.instance.OpenForm();
                    }
                }
            }
        }
        ...
        private void OnBuyPlayTime(CUIEvent uiEvent)
        {
            ...
            switch (this.CanBuyPlayTime(levelInfo, out coinType, out price, out reason))
            {
                ...
                default:
                    Singleton<CUIManager>.GetInstance().OpenTips(Singleton<CTextManager>.GetInstance().GetText("PVE_Level_Can_Not_Refresh_Level") + reason, false, 1f, null, new object[0]);
                    break;
            }
        }
        ...
        public void OnBuyPlayTimeRsp(CSPkg msg)
        {
            ...
            ResLevelCfgInfo dataByKey = GameDataMgr.levelDatabin.GetDataByKey((int) msg.stPkgData.stShopBuyRsp.stExtraInfo.dwLevelID);
            DebugHelper.Assert(dataByKey != null);
        ...
        private void OnChooseHeroReady(CUIEvent uiEvent)
        {
            if (uiEvent.m_srcWidget.GetComponent<Button>().interactable)
            {
                ...
                DebugHelper.Assert(GameDataMgr.levelDatabin.GetDataByKey(this.currentLevelId) != null);
        ...
        public static void OnGetChapterReward(CSPkg msg)
        {
            ...
            {
                Singleton<CUIManager>.GetInstance().OpenTips(Singleton<CTextManager>.GetInstance().GetText("PVE_Level_All_Chapter_Starts_Error") + Utility.ProtErrCodeToStr(0x421, msg.stPkgData.stGetChapterRewardRsp.iErrorCode), false, 1f, null, new object[0]);
            }
        }
            ...
            if (info != null)
            {
                ...
                switch (this.CanEnterLevel(this.currentChapter, this.currentLevelSeq, this.currentDifficulty))
                {
                    case EnterAdvError.Other:
                        Singleton<CUIManager>.instance.OpenTips("Level_Error_Tips_2", true, 1f, null, new object[0]);
                        break;
                    ...
                    case EnterAdvError.None:
                        ...
                        if (((this.currentChapter == 1) && (this.currentLevelSeq == 1)) && (this.currentDifficulty == 1))
                        {
                            ...
                            MonoSingleton<NewbieGuideManager>.GetInstance().SetNewbieBit(12, true);
                        }
                        break;
                    ...
                    case EnterAdvError.Locked:
                        ...
                        {
                            Singleton<CUIManager>.instance.OpenTips("Level_Error_Tips_1", true, 1f, null, new object[0]);
                        }
                        else
                        {
                            Singleton<CUIManager>.instance.OpenTips("Level_Error_Tips_3", true, 1f, null, new object[0]);
                        }
                        break;
                }
            }
        ...
        private void OnMopup(CUIEvent uiEvent)
        {
            if (Singleton<CFunctionUnlockSys>.instance.FucIsUnlock(RES_SPECIALFUNCUNLOCK_TYPE.RES_SPECIALFUNCUNLOCKTYPE_SAODANG))
            {
                ...
                {
                    Singleton<CUIManager>.GetInstance().OpenTips("Mopup_Condition", true, 1f, null, new object[0]);
                }
            }
            else
            {
        ...
        private void OnMopupTenTimes(CUIEvent uiEvent)
        {
            if (Singleton<CFunctionUnlockSys>.instance.FucIsUnlock(RES_SPECIALFUNCUNLOCK_TYPE.RES_SPECIALFUNCUNLOCKTYPE_SAODANG))
            {
                ...
                {
                    Singleton<CUIManager>.GetInstance().OpenTips("Mopup10_Condition", true, 1f, null, new object[0]);
                }
            }
            else
            {
        ...
        private void OnOpenChooseHero(CUIEvent uiEvent)
        {
            ...
            {
                object[] inParameters = new object[] { this.currentLevelId };
                DebugHelper.Assert(GameDataMgr.levelDatabin.GetDataByKey(this.currentLevelId) != null, "Can't find level config -- ID: {0}", inParameters);
                ...
                {
                    ResLevelCfgInfo dataByKey = GameDataMgr.levelDatabin.GetDataByKey(Singleton<CAdventureSys>.GetInstance().currentLevelId);
                    ...
                    stUIEventParams par = new stUIEventParams {
                        heroSelectGameType = enSelectHeroType.enPVE_Adventure
                    };
        ...
        public override void UnInit()
        {
            ...
            Singleton<EventRouter>.instance.RemoveEventHandler(EventID.SINGLEGAME_ERR_FREEHERO, new Action(this, (IntPtr) this.OnFreeHeroChanged));
            Singleton<EventRouter>.instance.RemoveEventHandler(EventID.GLOBAL_REFRESH_TIME, new Action(null, (IntPtr) ResetElitePlayNum));
        }
        ...
        public bool UpdateAdvProgress(bool bWin)
        {
            ...
            DebugHelper.Assert(dataByKey != null);
            int iChapterId = dataByKey.iChapterId;
            int bLevelNo = dataByKey.bLevelNo;
            bool flag = false;
            if (bWin)
            {
                if (this.IsLevelFinished(pveInfo, iChapterId, bLevelNo))
                {
                    pveInfo.ChapterDetailList[iChapterId - 1].LevelDetailList[bLevelNo - 1].levelStatus = 2;
                    PVE_LEVEL_COMPLETE_INFO pve_level_complete_info1 = pveInfo.ChapterDetailList[iChapterId - 1].LevelDetailList[bLevelNo - 1];
                    pve_level_complete_info1.PlayNum++;
                    byte starBits = Singleton<StarSystem>.GetInstance().GetStarBits();
                    if (this.HasMoreStar(starBits, pveInfo.ChapterDetailList[iChapterId - 1].LevelDetailList[bLevelNo - 1].bStarBits))
                    {
                        pveInfo.ChapterDetailList[iChapterId - 1].LevelDetailList[bLevelNo - 1].bStarBits = starBits;
                    }
                }
                else
                {
                    flag = true;
                    if (pveInfo.ChapterDetailList[iChapterId - 1] != null)
                    {
                        pveInfo.ChapterDetailList[iChapterId - 1].LevelDetailList[bLevelNo - 1].levelStatus = 2;
                        PVE_LEVEL_COMPLETE_INFO pve_level_complete_info2 = pveInfo.ChapterDetailList[iChapterId - 1].LevelDetailList[bLevelNo - 1];
                        pve_level_complete_info2.PlayNum++;
                        pveInfo.ChapterDetailList[iChapterId - 1].LevelDetailList[bLevelNo - 1].bStarBits = Singleton<StarSystem>.GetInstance().GetStarBits();
                    }
                    if (bLevelNo < 4)
                    {
                        if (pveInfo.ChapterDetailList[iChapterId - 1] != null)
                        {
                            pveInfo.ChapterDetailList[iChapterId - 1].LevelDetailList[bLevelNo].levelStatus = 1;
                            this.currentLevelSeq = bLevelNo + 1;
                        }
                    }
                    else if (this.currentDifficulty < LEVEL_DIFFICULT_OPENED)
                    {
                        ...
                        if (pve_adv_complete_info2.ChapterDetailList[iChapterId - 1] != null)
                        {
                            pve_adv_complete_info2.ChapterDetailList[iChapterId - 1].LevelDetailList[0].levelStatus = 1;
        ...
        [CompilerGenerated]
        private sealed class <GetLevelCfg>c__AnonStorey4D
        {
            internal int ChapterId;
            internal int Seq;

            internal bool <>m__54(ResLevelCfgInfo x)
            {
        ...
        [CompilerGenerated]
        private sealed class <GetLevelId>c__AnonStorey4E
        {
            internal int ChapterId;
            internal int Difficulty;
            internal int Seq;

            internal bool <>m__55(ResLevelCfgInfo x)
            {
