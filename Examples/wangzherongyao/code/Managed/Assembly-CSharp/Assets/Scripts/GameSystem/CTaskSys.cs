namespace Assets.Scripts.GameSystem
{
    ...
    public class CTaskSys : Singleton<CTaskSys>
    {
        ...
        private CTaskView m_taskView = new CTaskView();
        ...
        private void _check(CUIEvent uiEvent, bool bDay)
        {
            ...
            if (tagUInt != 0)
            {
                ...
                {
                    Singleton<CUIManager>.instance.OpenTips(UT.GetText("CS_HUOYUEDUREWARD_GETED"), false, 1f, null, new object[0]);
                }
                else
                {
        ...
        public void OnHuoyue_Error_NTF(ref CSPkg pkg)
        {
            ...
            {
                Singleton<CUIManager>.instance.OpenTips(text, false, 1f, null, new object[0]);
            }
        }
        ...
        public void OnLinkPveClick(CUIEvent uiEvent)
        {
            ...
            for (int i = 0; i < dataByKey.astPrerequisiteArray.Length; i++)
            {
                ...
                {
                    key = dataByKey.astPrerequisiteArray[i].astPrerequisiteParam[1].iParam;
                    break;
                }
            }
            if (key == 0)
            ...
            {
                ResLevelCfgInfo info = GameDataMgr.levelDatabin.GetDataByKey(key);
        ...
        public void OnTASKSUBMIT_RES(ref CSPkg pkg)
        {
            if (pkg.stPkgData.stSubmitTaskRes.bSubmitResult == 0)
            {
                ...
                if (task != null)
                {
                    ...
                    if (task.m_taskType == 0)
                    {
                        this.model.task_Data.Remove(task.m_baseId);
                    }
                }
            }
            Singleton<EventRouter>.instance.BroadCastEvent("TaskUpdated");
