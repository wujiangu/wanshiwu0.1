namespace Assets.Scripts.GameSystem
{
    ...
    public class CEquipSystem : Singleton<CEquipSystem>
    {
        ...
        private ushort[] GetDefaultRecommendEquipInfo(uint heroID)
        {
        ...
        private void LoadEquipListNode()
        {
            if (this.m_customEquipForm != null)
            {
                ...
                if (widget != null)
                {
                    Singleton<CMallSystem>.GetInstance().LoadUIPrefab(s_EquipListNodePrefabPath, "Panel_EquipList", widget, this.m_customEquipForm);
                }
            }
        }
        ...
        private void OnAddEquip(CUIEvent uiEvent)
        {
            ...
            {
                Singleton<CUIManager>.GetInstance().OpenTips("CustomEquip_ChooseEquipTip", true, 1f, null, new object[0]);
            }
            else
            {
        ...
        private void OnExpandCustomEquip(CUIEvent uiEvent)
        {
            if (null != this.m_customEquipForm)
            {
                ...
                {
                    <OnExpandCustomEquip>c__AnonStorey35 storey = new <OnExpandCustomEquip>c__AnonStorey35();
                    ...
                    LeanTween.value(widget, new Action<Vector2>(storey.<>m__29), storey.customContentRectTransform.anchoredPosition, to, this.c_moveAnimaTime);
                }
        ...
        {
            <OnPackUpCustomEquip>c__AnonStorey37 storey = new <OnPackUpCustomEquip>c__AnonStorey37();
            if (null != this.m_customEquipForm)
            {
                GameObject widget = this.m_customEquipForm.GetWidget(12);
                storey.equipCustomPanel = this.m_customEquipForm.GetWidget(11);
                if ((widget != null) && (storey.equipCustomPanel != null))
                {
                    <OnPackUpCustomEquip>c__AnonStorey36 storey2 = new <OnPackUpCustomEquip>c__AnonStorey36 {
                        <>f__ref$55 = storey
                    };
                    LeanTween.cancel(widget);
                    storey2.customContentRectTransform = widget.transform as RectTransform;
                    Vector2 to = new Vector2(storey2.customContentRectTransform.anchoredPosition.x, storey2.customContentRectTransform.anchoredPosition.y - this.m_uiCustomEquipContentHeight);
                    LeanTween.value(widget, new Action<Vector2>(storey2.<>m__2A), storey2.customContentRectTransform.anchoredPosition, to, this.c_moveAnimaTime).setOnComplete(new Action(storey2, (IntPtr) this.<>m__2B));
                }
                GameObject equipListNodeWidget = this.GetEquipListNodeWidget(enEquipListNodeWidget.EquipItemContent);
                if (equipListNodeWidget != null)
                {
        ...
        public void OpenTipsOnSvrRsp()
        {
            ...
            {
                this.m_useGodEquip = false;
                Singleton<CUIManager>.GetInstance().OpenTips("CustomEquip_UseGodEquipTip", true, 1f, null, new object[0]);
            }
            if (this.m_revertDefaultEquip)
            {
                this.m_revertDefaultEquip = false;
                Singleton<CUIManager>.GetInstance().OpenTips("CustomEquip_RevertDefaultTip", true, 1f, null, new object[0]);
            }
        }
        ...
        private void RefreshHeroListPanel()
        {
            ...
            if (null != form)
            {
                ...
                if (widget != null)
                {
                    bOwn = widget.GetComponent<Toggle>().isOn;
                }
                this.ResetHeroList(this.m_curHeroJob, bOwn);
        ...
        private void RefreshRightPanel(CEquipInfo equipInfo)
        {
            if ((null != this.m_customEquipForm) && (equipInfo != null))
            {
                ...
                if (widget != null)
                {
                    ...
                    widget.transform.Find("equipPropertyDescText").GetComponent<Text>().text = equipInfo.m_equipPropertyDesc;
                }
        ...
        private void ResetHeroList(enHeroJobType jobType, bool bOwn)
        {
            ...
            if (masterRoleInfo != null)
            {
                ...
                {
                    if (((jobType == enHeroJobType.All) || (allHeroList[i].bJob == ((byte) jobType))) && (!bOwn || masterRoleInfo.IsHaveHero(allHeroList[i].dwCfgID, false)))
                    {
        ...
        [CompilerGenerated]
        private sealed class <OnExpandCustomEquip>c__AnonStorey35
        {
            internal RectTransform customContentRectTransform;

            internal void <>m__29(Vector2 pos)
            {
        ...
        [CompilerGenerated]
        private sealed class <OnPackUpCustomEquip>c__AnonStorey36
        {
            internal CEquipSystem.<OnPackUpCustomEquip>c__AnonStorey37 <>f__ref$55;
            internal RectTransform customContentRectTransform;

            internal void <>m__2A(Vector2 pos)
            {
                this.customContentRectTransform.anchoredPosition = pos;
            }
            ...
            internal void <>m__2B()
            {
                this.<>f__ref$55.equipCustomPanel.CustomSetActive(false);
            }
        }

        [CompilerGenerated]
        private sealed class <OnPackUpCustomEquip>c__AnonStorey37
        {
