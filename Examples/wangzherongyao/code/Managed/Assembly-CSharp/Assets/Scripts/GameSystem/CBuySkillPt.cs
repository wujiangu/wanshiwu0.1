namespace Assets.Scripts.GameSystem
{
    ...
    internal class CBuySkillPt : CBuyPanelBase
    {
        ...
        public void Buy()
        {
            ...
            {
                Singleton<CUIManager>.GetInstance().OpenTips("没有足够的钻石！", false, 1f, null, new object[0]);
            }
            else if (!this.bSkillPointZero)
            {
                Singleton<CUIManager>.GetInstance().OpenTips("当前技能点不为零，不能继续购买！", false, 1f, null, new object[0]);
            }
        }
        ...
        {
            ResShopInfo dataByKey = GameDataMgr.resShopInfoDatabin.GetDataByKey((int) ((ushort) ((500 + base.m_CurBuyTime) + 1)));
            if (dataByKey != null)
            {
        ...
        protected override uint GetRequireDianquan(int times)
        {
            ...
            {
                dwCoinPrice = GameDataMgr.resShopInfoDatabin.GetDataByKey((int) ((ushort) (((500 + base.m_CurBuyTime) + 1) + i))).dwCoinPrice;
        ...
        public override void initPanel(CUIFormScript form)
        {
            ...
            this.RefreshUI();
            Singleton<EventRouter>.GetInstance().AddEventHandler("MasterAttributesChanged", new Action(this, (IntPtr) this.onAtrrChange));
        }
