namespace Assets.Scripts.GameSystem
{
    ...
    public class CHeroInfoSystem2 : Singleton<CHeroInfoSystem2>
    {
        ...
        private string GetSkinCannotBuyStr(ResHeroSkin skinInfo)
        {
            return (!string.IsNullOrEmpty(skinInfo.szGetPath) ? skinInfo.szGetPath : Singleton<CTextManager>.GetInstance().GetText("Hero_SkinState_CannotBuy"));
        }
        ...
        {
            if ((addReason == 1) && (Singleton<CUIManager>.instance.GetForm(CHeroSelectSystem.s_heroSelectFormPath) == null))
            {
        ...
        private void OnHeroInfo_MaterialDirectBuy(CUIEvent uiEvent)
        {
            ...
            if (masterRoleInfo != null)
            {
                ...
                {
                    Singleton<CUIManager>.GetInstance().OpenTips("Hero_Material_Direct_Buy_Limit_Tip", true, 1f, null, new object[0]);
                }
                else
                {
        ...
        private void OnHeroSkin_ItemEnable(CUIEvent uiEvent)
        {
            if (this.m_heroInfoForm != null)
            {
                ...
                if (null != widget)
                {
                    ...
                    {
                        ListView<ResHeroSkin> view = CSkinInfo.s_heroSkinDic[this.m_selectHeroID];
                        int selectedIndex = widget.transform.Find("List_Skin").GetComponent<CUIListScript>().GetSelectedIndex();
                        this.SetSkinListItem(uiEvent.m_srcWidget, view[srcWidgetIndexInBelongedList], srcWidgetIndexInBelongedList == selectedIndex);
                    }
                }
            }
        }
        ...
        {
            Singleton<CUIManager>.GetInstance().OpenTips(string.Format("PVP_Hero_Unlock_Tip", true, heroName), false, 1f, null, new object[0]);
        }
        ...
        private void OnViewHeroProperty(CUIEvent uiEvent)
        {
            ...
            if (script != null)
            {
                ...
                if ((masterRoleInfo != null) && masterRoleInfo.GetHeroInfo(this.m_selectHeroID, out info2, false))
                {
                    ...
                    {
                        ValueDataInfo[] actorValue = info2.mActorValue.GetActorValue();
                        int soulLevel = info2.mActorValue.SoulLevel;
                        RefreshBasePropPanel(gameObject, ref actorValue, soulLevel);
                        RefreshAtkPropPanel(root, ref actorValue, soulLevel, this.m_selectHeroID);
                        RefreshDefPropPanel(obj4, ref actorValue, soulLevel);
                    }
                }
                else
                {
                    SetHeroValueData(this.m_selectHeroID, ref CUICommonSystem.s_heroValArr);
                    RefreshBasePropPanel(gameObject, ref CUICommonSystem.s_heroValArr, 1);
                    RefreshAtkPropPanel(root, ref CUICommonSystem.s_heroValArr, 1, this.m_selectHeroID);
                    RefreshDefPropPanel(obj4, ref CUICommonSystem.s_heroValArr, 1);
                }
            }
        }
        ...
        {
            CUIFormScript script = Singleton<CUIManager>.GetInstance().OpenForm(s_heroStoryFormPath, false, true);
            if (script != null)
            {
                ResHeroCfgInfo dataByKey = GameDataMgr.heroDatabin.GetDataByKey(this.m_selectHeroID);
                script.transform.Find("Panel/titlebg/heroNameText").GetComponent<Text>().text = StringHelper.UTF8BytesToString(ref dataByKey.szName);
                script.transform.Find("Panel/Panel_Mask/heroStoryText").GetComponent<Text>().text = StringHelper.UTF8BytesToString(ref dataByKey.szStoryDesc);
            }
        }
        ...
        public static void OnWearHeroSkinRsp(CSPkg msg)
        {
            ...
            if (stWearHeroSkinRsp.iResult == 0)
            {
                ...
                Singleton<CHeroSelectSystem>.GetInstance().OnHeroSkinWearSuc(stWearHeroSkinRsp.dwHeroID, stWearHeroSkinRsp.dwSkinID);
            }
            else
            {
                ...
                switch (iResult)
                {
                    case CS_WEARHEROSKIN_ERRCODE.CS_WEARHEROSKIN_NOOWNEDHERO:
                        Singleton<CUIManager>.GetInstance().OpenTips(instance.GetText("Hero_SkinWear_Hero_Not_Exist"), false, 1f, null, new object[0]);
                        break;

                    case CS_WEARHEROSKIN_ERRCODE.CS_WEARHEROSKIN_NOOWNEDSKIN:
                        Singleton<CUIManager>.GetInstance().OpenTips(instance.GetText("Hero_SkinWear_Skin_Not_Exist"), false, 1f, null, new object[0]);
                        break;
                }
            }
        }
        ...
        private void RefreshBaseInfoPanel()
        {
            if (this.m_heroInfoForm != null)
            {
                ...
                if (null != widget)
                {
                    ...
                    GameObject obj4 = transform.Find("proficiencyTxt").gameObject;
                    Text component = transform.Find("proficiencyLevel").GetComponent<Text>();
                    Text text3 = transform.Find("proficiencyValTxt").GetComponent<Text>();
                    gameObject.CustomSetActive(this.m_selectHeroData.bPlayerOwn);
                    obj4.CustomSetActive(this.m_selectHeroData.bPlayerOwn);
                    component.gameObject.CustomSetActive(this.m_selectHeroData.bPlayerOwn);
                    text3.gameObject.CustomSetActive(this.m_selectHeroData.bPlayerOwn);
                    if (this.m_selectHeroData.bPlayerOwn)
                    {
                        ...
                        if (heroProficiency != null)
                        {
                            ...
                            {
                                text3.text = this.m_selectHeroData.proficiency.ToString();
                            }
                            else
                            {
                                text3.text = string.Format("{0}/{1}", this.m_selectHeroData.proficiency, heroProficiency.dwTopPoint);
                            }
        ...
        public static void RefreshBasePropPanel(GameObject root, ref ValueDataInfo[] info, int level)
        {
            Transform transform = root.transform;
            Text component = transform.Find("TextL1").GetComponent<Text>();
            Text text2 = transform.Find("TextR1").GetComponent<Text>();
            ...
            text15.text = Singleton<CTextManager>.GetInstance().GetText("Hero_Prop_MaxEp");
        ...
        private void RefreshBuyHeroPanel()
        {
            if (null != this.m_heroInfoForm)
            {
                ...
                if (!bPlayerOwn)
                {
                    ...
                    {
                        dataByKey.bIsBuyCoin = 1;
                        dataByKey.bIsBuyCoupons = 1;
                    }
                    ...
                    if (payInfoSet.m_payInfoCount <= 0)
                    {
                        ...
                        component.text = StringHelper.UTF8BytesToString(ref dataByKey.szObtWay);
                    }
        ...
        public static void RefreshDefPropPanel(GameObject root, ref ValueDataInfo[] info, int level)
        {
            Transform transform = root.transform;
            Text component = transform.Find("TextL1").GetComponent<Text>();
            Text text2 = transform.Find("TextR1").GetComponent<Text>();
            ...
            text5.text = Singleton<CTextManager>.GetInstance().GetText("Hero_Prop_EpRecover");
        ...
        public void SelectSkinElement(int index)
        {
            if (this.m_heroInfoForm != null)
            {
                ...
                if (null != widget)
                {
                    widget.transform.Find("List_Skin").GetComponent<CUIListScript>().SelectElement(index, true);
                }
            }
        }
        ...
        private static void SetHeroValueData(uint heroId, ref ValueDataInfo[] dataArr)
        {
            if (dataArr.Length == 0x24)
            {
                ...
                if (dataByKey != null)
                {
                    CBattleValueAdjust.SetPropValue(dataArr[5], (int) dataByKey.iBaseHP);
                    CBattleValueAdjust.SetPropValue(dataArr[1], (int) dataByKey.iBaseATT);
                    CBattleValueAdjust.SetPropValue(dataArr[2], (int) dataByKey.iBaseINT);
                    CBattleValueAdjust.SetPropValue(dataArr[0x12], (int) dataByKey.iBaseAtkSpd);
                    CBattleValueAdjust.SetPropValue(dataArr[3], (int) dataByKey.iBaseDEF);
                    CBattleValueAdjust.SetPropValue(dataArr[4], (int) dataByKey.iBaseRES);
                    CBattleValueAdjust.SetPropValue(dataArr[0x10], (int) dataByKey.iBaseHPAdd);
                    CBattleValueAdjust.SetPropValue(dataArr[6], (int) dataByKey.iCritRate);
                    CBattleValueAdjust.SetPropValue(dataArr[12], (int) dataByKey.iCritEft);
                    CBattleValueAdjust.SetPropValue(dataArr[15], (int) dataByKey.iBaseSpeed);
                }
            }
        }
            ...
            if ((listItem != null) && (skinInfo != null))
            {
                ...
                switch (Singleton<CRoleInfoManager>.GetInstance().GetMasterRoleInfo().GetHeroSkinState(this.m_selectHeroID, skinInfo.dwSkinID))
                {
                    ...
                    case HeroSkinState.NormalHero_LimitSkin_Wear:
                        ...
                        {
                            this.SetSkinStateText(component, this.GetSkinCannotBuyStr(skinInfo));
                            break;
                        }
