namespace Assets.Scripts.GameSystem
{
    ...
    public class CGuildInfoController : Singleton<CGuildInfoController>
    {
        ...
        public void OnAcceptInvite()
        {
            ...
            {
                Singleton<CUIManager>.GetInstance().OpenTips("Guild_No_Guild_Can_Accept_Invite_Tip", true, 1f, null, new object[0]);
            }
            else if (this.m_Model.m_InviteGuildUuid != 0)
            {
        ...
        public void OnGuildSymbolOperateSymbol(int operationType, uint symbolId)
        {
            Singleton<CUIManager>.GetInstance().OpenSendMsgAlert(5, enUIEventID.None);
            CSPkg msg = NetworkModule.CreateDefaultCSPKG(0x8dd);
            msg.stPkgData.stGuildSymbolReq.bOperType = (byte) operationType;
            msg.stPkgData.stGuildSymbolReq.dwSymbolBaseID = symbolId;
            Singleton<NetworkModule>.GetInstance().SendLobbyMsg(ref msg, false);
        }
        ...
        public void OnHyperLinkSearchGuild(CUIEvent uiEvent)
        {
            ...
            {
                Singleton<CUIManager>.GetInstance().OpenTips("Guild_Already_In_Normal_Guild", true, 1f, null, new object[0]);
            }
            else if (Singleton<CGuildSystem>.GetInstance().IsInPrepareGuild())
            {
                Singleton<CUIManager>.GetInstance().OpenTips("Guild_Already_In_Prepare_Guild", true, 1f, null, new object[0]);
            }
            else
            {
        ...
        private void RequestGroupGuildId()
        {
            ...
            Singleton<NetworkModule>.GetInstance().SendLobbyMsg(ref msg, false);
            Debug.LogError("Request 32 bit guild id");
        }
        ...
        private void RequestSetGuildGroupOpenId(string groupOpenId)
        {
            ...
            Singleton<NetworkModule>.GetInstance().SendLobbyMsg(ref msg, false);
            Debug.LogError("Send groupOpenId to server, groupOpenId=" + groupOpenId);
        }
