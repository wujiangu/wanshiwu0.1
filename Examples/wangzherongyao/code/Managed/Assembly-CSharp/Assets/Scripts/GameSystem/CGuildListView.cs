namespace Assets.Scripts.GameSystem
{
    ...
    public class CGuildListView : Singleton<CGuildListView>
    {
        ...
        private void On_Guild_PrepareGuild_Create(CUIEvent uiEvent)
        {
            ...
            {
                uint num2 = GameDataMgr.guildMiscDatabin.GetDataByKey(3).dwConfValue;
                if (num < (Singleton<CRoleInfoManager>.GetInstance().GetMasterRoleInfo().m_extGuildInfo.dwLastCreateGuildTime + num2))
                {
                    Singleton<CUIManager>.GetInstance().OpenTips("Guild_Last_Create_Time_Tip", true, 1f, null, new object[0]);
                    return;
                }
            }
            CRoleInfo masterRoleInfo = Singleton<CRoleInfoManager>.GetInstance().GetMasterRoleInfo();
            uint dwConfValue = GameDataMgr.guildMiscDatabin.GetDataByKey(5).dwConfValue;
            ...
            {
                uint num4 = GameDataMgr.guildMiscDatabin.GetDataByKey(10).dwConfValue;
                ...
                {
                    string str = this.m_form.GetWidget(0).GetComponent<Text>().text.Trim();
                    ...
                    else if (string.IsNullOrEmpty(this.m_form.GetWidget(1).GetComponent<Text>().text.Trim()))
                    {
                        Singleton<CUIManager>.GetInstance().OpenTips("Guild_Input_Guild_Bulletin_Empty", true, 1f, null, new object[0]);
                    }
                    else
                    {
                        uint num5 = GameDataMgr.guildMiscDatabin.GetDataByKey(2).dwConfValue;
                        uint num6 = GameDataMgr.guildMiscDatabin.GetDataByKey(1).dwConfValue;
        ...
        private void On_Guild_PrepareGuild_Create_Confirm(CUIEvent uiEvent)
        {
            if (this.m_Model.IsInGuildStep())
            {
                Singleton<CUIManager>.GetInstance().OpenTips("Guild_In_Guild_Step_Tip_2", true, 1f, null, new object[0]);
            }
            else
            {
        ...
        {
            <OpenIconForm>c__AnonStorey3E storeye = new <OpenIconForm>c__AnonStorey3E {
                <>f__this = this
            };
            CUIFormScript script = Singleton<CUIManager>.GetInstance().OpenForm("UGUI/Form/System/Guild/Form_Guild_Icon.prefab", false, true);
            storeye.listScript = script.GetWidget(0).GetComponent<CUIListScript>();
            int count = GameDataMgr.guildIconDatabin.count;
            storeye.listScript.SetElementAmount(count);
            storeye.i = 0;
            GameDataMgr.guildIconDatabin.Accept(new Action<ResGuildIcon>(storeye.<>m__34));
        }
        ...
        private void RefreshPrepareGuildPanelMemberList()
        {
            ...
            uint dwConfValue = GameDataMgr.guildMiscDatabin.GetDataByKey(1).dwConfValue;
            dwConfValue = (dwConfValue >= 0) ? dwConfValue : 0;
            CUITimerScript script4 = this.m_form.GetWidget(20).GetComponent<CUITimerScript>();
            uint num5 = this.m_Model.CurrentPrepareGuildInfo.stBriefInfo.dwRequestTime + GameDataMgr.guildMiscDatabin.GetDataByKey(2).dwConfValue;
            int seconds = ((int) num5) - Singleton<CRoleInfoManager>.GetInstance().GetMasterRoleInfo().getCurrentTimeSinceLogin();
            if (seconds < 0)
            {
                seconds = 0;
            }
        ...
        private void SetGuildListItem(CUIListElementScript listElementScript, GuildInfo info)
        {
            ...
            Transform rankTransform = transform.Find("rank").transform;
            CGuildHelper.SetRankDisplay(info.stBriefInfo.Rank, rankTransform);
        ...
        private void SetPrepareGuildListItem(CUIListElementScript listElementScript, PrepareGuildInfo info)
        {
            ...
            text3.text = (info.stBriefInfo.bMemCnt - 1) + "/" + GameDataMgr.guildMiscDatabin.GetDataByKey(1).dwConfValue;
            uint num = info.stBriefInfo.dwRequestTime + GameDataMgr.guildMiscDatabin.GetDataByKey(2).dwConfValue;
            int seconds = ((int) num) - Singleton<CRoleInfoManager>.GetInstance().GetMasterRoleInfo().getCurrentTimeSinceLogin();
            if (seconds < 0)
            {
                seconds = 0;
            }
        ...
        [CompilerGenerated]
        private sealed class <OpenIconForm>c__AnonStorey3E
        {
            internal CGuildListView <>f__this;
            internal int i;
            internal CUIListScript listScript;

            internal void <>m__34(ResGuildIcon x)
            {
