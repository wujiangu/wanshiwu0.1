namespace Assets.Scripts.GameSystem
{
    ...
    public class CMallMysteryShop : Singleton<CMallMysteryShop>
    {
        ...
        public static string MYSTERY_TIPS_FORM_PATH = "UGUI/Form/System/Mall/Form_Mystery_Tips.prefab";
        ...
        public int GetTimeToClose()
        {
            ...
            {
                TimeSpan span = (TimeSpan) (this.m_EndTime - Utility.ToUtcTime2Local((long) masterRoleInfo.getCurrentTimeSinceLogin()));
        ...
        {
            Singleton<CMallSystem>.GetInstance().LoadUIPrefab("UGUI/Form/System/Mall/Mystery", "pnlMystery", form.GetWidget(3), form);
        }
        ...
        private void OnConfirmuyItem(CUIEvent uiEvent)
        {
            ...
            if (tag < 0)
            {
                ...
                Singleton<CUIManager>.GetInstance().OpenTips("该商品无法购买", false, 1f, null, new object[0]);
            }
            else if (this.BoughtCnt >= this.LimitBuyCnt)
            {
                Singleton<CUIManager>.GetInstance().OpenTips("您的购买次数已达到神秘商店限购的次数，欢迎下次再来", false, 1f, null, new object[0]);
            }
            else
            {
        ...
        {
            Singleton<CUIManager>.GetInstance().OpenTips("神秘商店已关闭", false, 1f, null, new object[0]);
        ...
        {
            Singleton<CUIManager>.GetInstance().CloseSendMsgAlert();
            Singleton<CUIManager>.GetInstance().OpenTips(Utility.ProtErrCodeToStr(0x580, msg.stPkgData.stAkaliShopError.iErrorCode), false, 1f, null, new object[0]);
        }
        ...
        {
            <RefreshProducts>c__AnonStorey49 storey = new <RefreshProducts>c__AnonStorey49 {
                role = Singleton<CRoleInfoManager>.GetInstance().GetMasterRoleInfo()
            };
            DebugHelper.Assert(storey.role != null, "master roleInfo is null");
            if (storey.role != null)
            {
                if (this.m_Products != null)
                {
                    this.m_Products.Sort(new Comparison<CMallMysteryProduct>(storey.<>m__47));
                }
                CUIFormScript mallForm = Singleton<CMallSystem>.GetInstance().m_MallForm;
                if ((mallForm != null) && Singleton<CMallSystem>.GetInstance().m_IsMallFormOpen)
                {
        ...
        [CompilerGenerated]
        private sealed class <RefreshProducts>c__AnonStorey49
        {
            internal CRoleInfo role;

            internal int <>m__47(CMallMysteryProduct l, CMallMysteryProduct r)
            {
                if (l == null)
                {
                    return -1;
