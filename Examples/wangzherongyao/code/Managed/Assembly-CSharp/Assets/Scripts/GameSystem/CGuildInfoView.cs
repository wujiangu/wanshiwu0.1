    ...
    public class CGuildInfoView : Singleton<CGuildInfoView>
    {
        ...
        public void BindQQGroup()
        {
            ...
            if (iApolloSnsService != null)
            {
                ...
                Debug.LogError("绑定QQ群: guild32BitsUid=" + cUnionid + "\nguildName=" + guildName + "\nguildZoneId=" + cZoneid + "\nguildSignatureMd5=" + bindQQGroupSignature);
            }
        }
        ...
        public override void Init()
        {
            ...
            Singleton<CUIEventManager>.GetInstance().AddUIEventListener(enUIEventID.Guild_Construct_Upgrade, new CUIEventManager.OnUIEventHandler(this.OnGuildConstructUpgrade));
            Singleton<CUIEventManager>.GetInstance().AddUIEventListener(enUIEventID.Guild_Construct_View, new CUIEventManager.OnUIEventHandler(this.OnGuildConstructView));
            Singleton<CUIEventManager>.GetInstance().AddUIEventListener(enUIEventID.Guild_Construct_Guide, new CUIEventManager.OnUIEventHandler(this.OnGuildConstructGuide));
            ...
            Singleton<CUIEventManager>.GetInstance().AddUIEventListener(enUIEventID.Guild_Symbol_Open_Symbol_Form, new CUIEventManager.OnUIEventHandler(this.On_Guild_Symbol_Open_Symbol_Form));
            Singleton<CUIEventManager>.GetInstance().AddUIEventListener(enUIEventID.Guild_Symbol_Open_Or_Upgrade_Symbol, new CUIEventManager.OnUIEventHandler(this.On_Guild_Symbol_Open_Or_Upgrade_Symbol));
            Singleton<CUIEventManager>.GetInstance().AddUIEventListener(enUIEventID.Guild_Symbol_Symbol_List_Select_Change, new CUIEventManager.OnUIEventHandler(this.On_Guild_Symbol_Symbol_List_Select_Change));
            Singleton<CUIEventManager>.GetInstance().AddUIEventListener(enUIEventID.Guild_Symbol_Open_Rule, new CUIEventManager.OnUIEventHandler(this.On_Guild_Symbol_Open_Rule));
        ...
        {
            FriendSysNetCore.Send_Request_BeFriend(this.m_Model.CurrentSelectedMemberInfo.stBriefInfo.uulUid, (uint) this.m_Model.CurrentSelectedMemberInfo.stBriefInfo.dwLogicWorldId);
        }
        ...
        private void On_Guild_Confirm_Modify_Guild_Bulletin(CUIEvent uiEvent)
        {
            ...
            if (srcFormScript != null)
            {
                string str = srcFormScript.GetWidget(1).GetComponent<Text>().text.Trim();
                if (string.IsNullOrEmpty(str))
                {
                    Singleton<CUIManager>.GetInstance().OpenTips("Guild_Input_Guild_Bulletin_Empty", true, 1f, null, new object[0]);
                }
                else
                {
        ...
        private void On_Guild_Extend_Member_Limit(CUIEvent uiEvent)
        {
            ...
            {
                Singleton<CUIManager>.GetInstance().OpenTips("Guild_Member_Count_Reach_Limit", true, 1f, null, new object[0]);
            }
            else
            {
        ...
        private void On_Guild_Get_Guild_Dividend(CUIEvent uiEvent)
        {
            ...
            {
                Singleton<CUIManager>.GetInstance().OpenTips("Guild_No_Dividend_Can_Get", true, 1f, null, new object[0]);
            }
            else
            {
        ...
        {
            if ((Singleton<CRoleInfoManager>.GetInstance().GetMasterRoleInfo().m_baseGuildInfo.guildState == COM_PLAYER_GUILD_STATE.COM_PLAYER_GUILD_STATE_CHAIRMAN) && (this.m_Model.CurrentGuildInfo.stBriefInfo.bMemCnt > 1))
            {
                Singleton<CUIManager>.GetInstance().OpenTips("Guild_ChairMan_Quit_Tip", true, 1f, null, new object[0]);
            }
            else
            {
                string text;
                uint dwConfValue = GameDataMgr.guildMiscDatabin.GetDataByKey(7).dwConfValue;
                TimeSpan span = new TimeSpan(0, 0, 0, (int) dwConfValue);
                GuildMemInfo playerGuildMemberInfo = this.m_Model.GetPlayerGuildMemberInfo();
                uint num2 = (playerGuildMemberInfo != null) ? ((playerGuildMemberInfo.dwConstruct * GameDataMgr.guildMiscDatabin.GetDataByKey(20).dwConfValue) / 100) : 0;
        ...
        private void On_Guild_JoinQQGroup(CUIEvent uiEvent)
        {
            ...
            if (iApolloSnsService != null)
            {
                if (!string.IsNullOrEmpty(this.m_Model.CurrentGuildInfo.groupKey))
                {
                    ...
                    Debug.LogError("申请加入QQ群: groupKey=" + this.m_Model.CurrentGuildInfo.groupKey);
                    Singleton<CUIManager>.GetInstance().OpenTips("Guild_QQGroup_Join_Tip", true, 1f, null, new object[0]);
                }
                else
                {
                    Debug.LogError("申请加入QQ群: group key is null!!!");
                }
            }
        }
        ...
        private void On_Guild_Need_Approval_Slider_Value_Changed(CUIEvent uiEvent)
        {
            ...
            if (srcFormScript != null)
            {
                int sliderValue = uiEvent.m_eventParams.sliderValue;
        ...
        public void On_Guild_Symbol_Open_Or_Upgrade_Symbol(CUIEvent uiEvent)
        {
            int tag = uiEvent.m_eventParams.tag;
            uint num2 = uiEvent.m_eventParams.commonUInt32Param1;
            Singleton<EventRouter>.GetInstance().BroadCastEvent<int, uint>("Guild_Symbol_Operate_Symbol", tag, num2);
        }

        public void On_Guild_Symbol_Open_Rule(CUIEvent uiEvent)
        {
            Singleton<CUIManager>.GetInstance().OpenMessageBox(Singleton<CTextManager>.GetInstance().GetText("Guild_Symbol_Rule_Content"), false);
        }

        public void On_Guild_Symbol_Open_Symbol_Form(CUIEvent uiEvent)
        {
            CUIFormScript symbolForm = Singleton<CUIManager>.GetInstance().OpenForm("UGUI/Form/System/Guild/Form_Guild_Symbol.prefab", false, true);
            this.RefreshSymbolForm(symbolForm);
        }

        public void On_Guild_Symbol_Symbol_List_Select_Change(CUIEvent uiEvent)
        {
            CUIFormScript srcFormScript = uiEvent.m_srcFormScript;
            if (srcFormScript != null)
            {
                CUIListScript component = uiEvent.m_srcWidget.GetComponent<CUIListScript>();
                uint num = uint.Parse(component.GetSelectedElement().transform.Find("txtIdData").GetComponent<Text>().text);
                Image image = srcFormScript.GetWidget(1).GetComponent<Image>();
                Text text = srcFormScript.GetWidget(2).GetComponent<Text>();
                Text text2 = srcFormScript.GetWidget(3).GetComponent<Text>();
                Text text3 = srcFormScript.GetWidget(4).GetComponent<Text>();
                Text text4 = srcFormScript.GetWidget(5).GetComponent<Text>();
                Text text5 = srcFormScript.GetWidget(6).GetComponent<Text>();
                GameObject widget = srcFormScript.GetWidget(7);
                List<uint> levelOneGuildSymbolIds = CGuildHelper.GetLevelOneGuildSymbolIds();
                ResSymbolInfo dataByKey = GameDataMgr.symbolInfoDatabin.GetDataByKey((num <= 0) ? levelOneGuildSymbolIds[component.GetSelectedIndex()] : num);
                string prefabPath = CUIUtility.s_Sprite_Dynamic_Icon_Dir + dataByKey.dwIcon;
                image.SetSprite(prefabPath, srcFormScript, true, false, false);
                text.text = StringHelper.UTF8BytesToString(ref dataByKey.szDisplayName);
                if (num > 0)
                {
                    if (dataByKey.wLevel == CGuildHelper.GetSerialGuildSymbolCount(levelOneGuildSymbolIds[component.GetSelectedIndex()]))
                    {
                        string[] args = new string[] { dataByKey.wLevel.ToString() };
                        text2.text = Singleton<CTextManager>.GetInstance().GetText("Guild_Symbol_Level_Format_2", args);
                        text3.text = StringHelper.UTF8BytesToString(ref dataByKey.szDesc);
                        text4.text = string.Empty;
                        widget.CustomSetActive(false);
                    }
                    else
                    {
                        string[] textArray2 = new string[] { dataByKey.wLevel.ToString(), (dataByKey.wLevel + 1).ToString() };
                        text2.text = Singleton<CTextManager>.GetInstance().GetText("Guild_Symbol_Upgrade_Level_Process", textArray2);
                        text3.text = CGuildHelper.GetSymbolAttUpgradeText(dataByKey);
                        ResSymbolInfo symbolInfo = GameDataMgr.symbolInfoDatabin.GetDataByKey(dataByKey.dwNextLvID);
                        text4.text = CGuildHelper.GetSymbolConditionText(symbolInfo, true);
                        widget.CustomSetActive(true);
                        stUIEventParams eventParams = new stUIEventParams {
                            tag = 2,
                            commonUInt32Param1 = dataByKey.dwID
                        };
                        widget.GetComponent<CUIEventScript>().SetUIEvent(enUIEventType.Click, enUIEventID.Guild_Symbol_Open_Or_Upgrade_Symbol, eventParams);
                        text5.text = Singleton<CTextManager>.GetInstance().GetText("Common_Up_Level");
                    }
                }
                else
                {
                    string[] textArray3 = new string[] { dataByKey.wLevel.ToString() };
                    text2.text = Singleton<CTextManager>.GetInstance().GetText("Guild_Symbol_Level_Format_2", textArray3);
                    text3.text = StringHelper.UTF8BytesToString(ref dataByKey.szDesc);
                    text4.text = CGuildHelper.GetSymbolConditionText(dataByKey, false);
                    widget.CustomSetActive(true);
                    stUIEventParams params2 = new stUIEventParams {
                        tag = 1,
                        commonUInt32Param1 = dataByKey.dwID
                    };
                    widget.GetComponent<CUIEventScript>().SetUIEvent(enUIEventType.Click, enUIEventID.Guild_Symbol_Open_Or_Upgrade_Symbol, params2);
                    text5.text = Singleton<CTextManager>.GetInstance().GetText("Guild_Symbol_Open");
                }
            }
        }
        ...
        private void On_Guild_UnBindQQGroup(CUIEvent uiEvent)
        {
            ...
            if (iApolloSnsService != null)
            {
                ...
                if (!string.IsNullOrEmpty(groupOpenId))
                {
                    ...
                    Debug.LogError("解绑QQ群: guildOpenId=" + groupOpenId + ", guild32BitsUid=" + cUnionid);
                }
                else
                {
                    Debug.LogError("解绑QQ群: guildOpenId is null!!!");
                }
            }
        }
        ...
        {
            this.QueryQQGroupInfo();
        }

        private void OnGuildConstructGuide(CUIEvent uiEvent)
        {
            string text = Singleton<CTextManager>.GetInstance().GetText("Guild_Construct_Guide");
            string constructGuide = CGuildHelper.GetConstructGuide(this.m_Model.CurrentGuildInfo.stBriefInfo.bLevel);
            Singleton<CUIManager>.GetInstance().OpenInfoForm(text, constructGuide);
        }

        private void OnGuildConstructUpgrade(CUIEvent uiEvent)
        {
            CUIListScript srcWidgetBelongedListScript = uiEvent.m_srcWidgetBelongedListScript;
            if (srcWidgetBelongedListScript != null)
            {
                CUIListElementScript elemenet = srcWidgetBelongedListScript.GetElemenet(uiEvent.m_srcWidgetIndexInBelongedList);
                if (elemenet != null)
                {
                    if (!CGuildSystem.HasManageAuthority())
                    {
                        Singleton<CUIManager>.GetInstance().OpenTips("Guild_No_Operate_Authority", true, 1f, null, new object[0]);
                    }
                    else
                    {
                        int buildingType = int.Parse(elemenet.transform.Find("txtTypeData").GetComponent<Text>().text);
                        int curBuildingLevel = int.Parse(elemenet.transform.Find("txtLevelData").GetComponent<Text>().text);
                        if (this.m_Model.CurrentGuildInfo.dwGuildMoney < CGuildHelper.GetSingleBuildingUpgradeMoney(buildingType, curBuildingLevel))
                        {
                            Singleton<CUIManager>.GetInstance().OpenTips("Guild_Guild_Money_Not_Enough", true, 1f, null, new object[0]);
                        }
                        else
                        {
                            if (buildingType == 1)
                            {
                                if (CGuildHelper.GetBuildingLevelSumExceptHall() < CGuildHelper.GetGuildUpgradeNeedOtherBuildingLevelSum())
                                {
                                    Singleton<CUIManager>.GetInstance().OpenTips("Guild_Upgrade_Failed_Other_Building_Level_Sum_Not_Enough", true, 1f, null, new object[0]);
                                    return;
                                }
                            }
                            else if ((curBuildingLevel + 1) > CGuildHelper.GetOtherBuildingCanReachMaxLevel())
                            {
                                Singleton<CUIManager>.GetInstance().OpenTips("Guild_Upgrade_Failed_Hall_Level_Not_Enough", true, 1f, null, new object[0]);
                                return;
                            }
                            Singleton<EventRouter>.GetInstance().BroadCastEvent<int>("Guild_Building_Upgrade", buildingType);
                        }
                    }
                }
            }
        ...
        private void OnGuildConstructView(CUIEvent uiEvent)
        {
            CUIListScript srcWidgetScript = uiEvent.m_srcWidgetScript as CUIListScript;
            if (srcWidgetScript != null)
            {
                CUIListElementScript selectedElement = srcWidgetScript.GetSelectedElement();
                if (selectedElement != null)
                {
                    string buildingInfo;
                    int num3;
                    int buildingType = int.Parse(selectedElement.transform.Find("txtTypeData").GetComponent<Text>().text);
                    int buildingLevel = int.Parse(selectedElement.transform.Find("txtLevelData").GetComponent<Text>().text);
                    bool isShowUpgradeButton = true;
                    if (CGuildHelper.IsBuildingUnlocked(buildingType, out num3))
                    {
                        buildingInfo = CGuildHelper.GetBuildingInfo(buildingType, buildingLevel);
                        if (buildingLevel == 0x13)
                        {
                            isShowUpgradeButton = false;
                        }
                    }
                    else
                    {
                        string[] args = new string[] { num3.ToString() };
                        buildingInfo = Singleton<CTextManager>.GetInstance().GetText("Guild_Building_Unlock_Condition", args);
                        isShowUpgradeButton = false;
                    }
                    string buildingName = CGuildHelper.GetBuildingName(buildingType);
                    string buildingGuide = CGuildHelper.GetBuildingGuide(buildingType);
                    string iconPath = CUIUtility.s_Sprite_Dynamic_BustHero_Dir + "30105";
                    this.OpenBuildingDetailForm(buildingName, buildingInfo, buildingGuide, iconPath, isShowUpgradeButton);
                }
            }
        }
        ...
        private void OnGuildPositionAppointViceChairman(CUIEvent uiEvent)
        {
            ...
            {
                Singleton<CUIManager>.GetInstance().OpenTips("Guild_Appoint_You_Have_No_Authority", true, 1f, null, new object[0]);
            }
            else if (!CGuildSystem.CanBeAppointedToViceChairman(this.m_Model.CurrentSelectedMemberInfo.enPosition))
            {
                Singleton<CUIManager>.GetInstance().OpenTips("Guild_Appoint_Vice_Chairman_Position_Limit_Tip", true, 1f, null, new object[0]);
            }
        ...
        private void OnGuildPositionChairmanTransfer(CUIEvent uiEvent)
        {
            ...
            {
                Singleton<CUIManager>.GetInstance().OpenTips("Guild_Only_Chairman_Can_Transfer_Position", true, 1f, null, new object[0]);
            }
            ...
            {
                Singleton<CUIManager>.GetInstance().OpenTips("Guild_Cannot_Transfer_To_Self", true, 1f, null, new object[0]);
            }
            else
            {
        ...
        private void OnGuildPositionFireMember(CUIEvent uiEvent)
        {
            ...
            {
                Singleton<CUIManager>.GetInstance().OpenTips("Guild_Fire_Member_You_Have_No_Authority", true, 1f, null, new object[0]);
            }
            else
            {
                string sName = this.m_Model.CurrentSelectedMemberInfo.stBriefInfo.sName;
        ...
        private void OnQueryQQGroupInfoNotify(ApolloGroupResult groupRet)
        {
            ...
            if (groupRet.result == ApolloResult.Success)
            {
                ...
                string[] textArray1 = new string[] { "查询群消息成功! groupName=", groupRet.groupInfo.groupName, "\ngroupOpenId=", groupRet.groupInfo.groupOpenid, "\ngroupKey=", groupRet.groupInfo.groupKey };
        ...
        private void OnQueryQQGroupKeyNotify(ApolloGroupResult groupRet)
        {
            ...
            {
                this.m_Model.CurrentGuildInfo.groupKey = groupRet.groupInfo.groupKey;
                Debug.LogError("查询QQ群Key成功: groupKey=" + this.m_Model.CurrentGuildInfo.groupKey);
            }
            else
            {
                Debug.LogError("查询QQ群Key失败, please try again!");
            }
        }
        ...
        private void OpenDonateForm()
        {
            ...
            if (script != null)
            {
                uint dwConfValue = GameDataMgr.guildMiscDatabin.GetDataByKey(0x13).dwConfValue;
        ...
        {
            <OpenGuildIconForm>c__AnonStorey3D storeyd = new <OpenGuildIconForm>c__AnonStorey3D {
                <>f__this = this
            };
            CUIFormScript script = Singleton<CUIManager>.GetInstance().OpenForm("UGUI/Form/System/Guild/Form_Guild_Icon.prefab", false, true);
            storeyd.listScript = script.GetWidget(0).GetComponent<CUIListScript>();
            int count = GameDataMgr.guildIconDatabin.count;
            storeyd.listScript.SetElementAmount(count);
            storeyd.i = 0;
            GameDataMgr.guildIconDatabin.Accept(new Action<ResGuildIcon>(storeyd.<>m__31));
            storeyd.listScript.SetElementSelectChangedEvent(enUIEventID.Guild_Guild_Setting_Guild_Icon_Selected);
        }
        ...
        public void QueryQQGroupInfo()
        {
            ...
            if (iApolloSnsService != null)
            {
                string cUnionid = CGuildHelper.GetGroupGuildId().ToString();
                string cZoneid = CGuildHelper.GetGuildLogicWorldId().ToString();
                Debug.LogError("查询群信息: guild32bitsUid=" + cUnionid + ", zoneId=" + cZoneid);
        ...
        private void QueryQQGroupKey()
        {
            ...
            if (iApolloSnsService != null)
            {
                Debug.LogError("查询QQ群Key: groupOpenId=" + this.m_Model.CurrentGuildInfo.groupOpenId);
        ...
        public void RefreshSymbolForm(CUIFormScript symbolForm = null)
        {
            if (symbolForm == null)
            {
                symbolForm = Singleton<CUIManager>.GetInstance().GetForm("UGUI/Form/System/Guild/Form_Guild_Symbol.prefab");
            }
            List<uint> levelOneGuildSymbolIds = CGuildHelper.GetLevelOneGuildSymbolIds();
            CUIListScript component = symbolForm.GetWidget(0).GetComponent<CUIListScript>();
            component.SetElementAmount(levelOneGuildSymbolIds.Count);
            for (int i = 0; i < levelOneGuildSymbolIds.Count; i++)
            {
                CUIListElementScript elemenet = component.GetElemenet(i);
                Text text = elemenet.transform.Find("txtIdData").GetComponent<Text>();
                Image image = elemenet.transform.Find("imgIcon").GetComponent<Image>();
                Text text2 = elemenet.transform.Find("txtName").GetComponent<Text>();
                Text text3 = elemenet.transform.Find("txtAtt").GetComponent<Text>();
                Text text4 = elemenet.transform.Find("txtLevel").GetComponent<Text>();
                uint bagGuildSymbolIdByLevelOneSymbolId = CGuildHelper.GetBagGuildSymbolIdByLevelOneSymbolId(levelOneGuildSymbolIds[i]);
                text.text = bagGuildSymbolIdByLevelOneSymbolId.ToString();
                ResSymbolInfo info = (bagGuildSymbolIdByLevelOneSymbolId <= 0) ? GameDataMgr.symbolInfoDatabin.GetDataByKey(levelOneGuildSymbolIds[i]) : GameDataMgr.symbolInfoDatabin.GetDataByKey(bagGuildSymbolIdByLevelOneSymbolId);
                string prefabPath = CUIUtility.s_Sprite_Dynamic_Icon_Dir + info.dwIcon;
                image.SetSprite(prefabPath, symbolForm, true, false, false);
                text2.text = StringHelper.UTF8BytesToString(ref info.szDisplayName);
                text3.text = StringHelper.UTF8BytesToString(ref info.szDesc);
                string str2 = (bagGuildSymbolIdByLevelOneSymbolId <= 0) ? Singleton<CTextManager>.GetInstance().GetText("Guild_Symbol_Not_Open") : Singleton<CTextManager>.GetInstance().GetText("Guild_Symbol_Level_Format", new string[] { info.wLevel.ToString(), CGuildHelper.GetSerialGuildSymbolCount(levelOneGuildSymbolIds[i]).ToString() });
                text4.text = str2;
            }
            if (component.GetElementAmount() > 0)
            {
                if (component.GetSelectedIndex() >= 0)
                {
                    component.SelectElement(component.GetSelectedIndex(), true);
                }
                else
                {
                    component.SelectElement(0, true);
        ...
        private void SetApplicantItem(CUIListElementScript listElementScript, stApplicantInfo info, bool isAdmin)
        {
            ...
            int num2 = 0;
            int dwConfValue = (int) GameDataMgr.guildMiscDatabin.GetDataByKey(9).dwConfValue;
            ...
            {
                num2 = (info.dwApplyTime + dwConfValue) - num;
                if (num2 < 0)
                {
                    num2 = 0;
                }
        ...
        private void SetGuildMemberListItem(CUIListElementScript listElementScript, GuildMemInfo memberInfo)
        {
            ...
            Text text4 = transform.Find("txtAbility").GetComponent<Text>();
            Text text5 = transform.Find("txtWeekActive").GetComponent<Text>();
            Text text6 = transform.Find("txtTotalConstruct").GetComponent<Text>();
            Text text7 = transform.Find("txtOnline").GetComponent<Text>();
            ...
            text4.text = memberInfo.stBriefInfo.dwAbility.ToString();
            text5.text = memberInfo.WeekActive.ToString();
            text6.text = memberInfo.TotalContruct.ToString();
            if (CGuildHelper.IsMemberOnline(memberInfo))
            {
                text7.text = Singleton<CTextManager>.GetInstance().GetText("Common_Online");
            }
            else
            {
                text7.text = Utility.GetRecentOnlineTimeString((int) memberInfo.LastLoginTime);
            }
        }
        ...
        private void SetPreviewGuildListItem(CUIListElementScript listElementScript, GuildInfo info)
        {
            ...
            if (info.stBriefInfo.uulUid == this.m_Model.m_InviteGuildUuid)
            {
                gameObject.CustomSetActive(true);
            }
            else
        ...
        private void SetRankpointGuildRankListItem(CUIListElementScript listElement, RankpointRankInfo info, enGuildRankpointRankListType rankListType, uint firstRankNo)
        {
            if ((listElement != null) && (info != null))
            {
                ...
                CGuildHelper.SetRankDisplay(info.rankNo - this.CalculateRankNoOffset(rankListType, firstRankNo), transform);
        ...
        {
            if ((itemTransform != null) && (info != null))
            {
                ...
                CGuildHelper.SetRankDisplay((uint) (elementIndex + 1), rankTransform);
        ...
        public void SetRankpointPlayerGuildRank(RankpointRankInfo info, CUIFormScript rankForm = null)
        {
            ...
            if ((rankForm != null) && (info != null))
            {
                ...
                if (((rankListTypeByTabSelectedIndex == enGuildRankpointRankListType.SeasonSelf) && (info.rankNo > 0)) && (this.m_Model.RankpointRankInfoLists[(int) rankListTypeByTabSelectedIndex].Count > 0))
                {
                    ...
                    CGuildHelper.SetRankDisplay(info.rankNo - this.CalculateRankNoOffset(rankListTypeByTabSelectedIndex, rankNo), rankTransform);
                }
                else
                {
                    CGuildHelper.SetRankDisplay(info.rankNo, rankTransform);
                }
        ...
        private void SortGuildMemberList()
        {
            if (<>f__am$cache4 == null)
            {
                <>f__am$cache4 = delegate (GuildMemInfo info1, GuildMemInfo info2) {
                    ...
                    {
                        return -info1.stBriefInfo.dwLevel.CompareTo(info2.stBriefInfo.dwLevel);
                    }
        ...
        [CompilerGenerated]
        private sealed class <OpenGuildIconForm>c__AnonStorey3D
        {
            internal CGuildInfoView <>f__this;
            internal int i;
            internal CUIListScript listScript;

            internal void <>m__31(ResGuildIcon x)
            {
