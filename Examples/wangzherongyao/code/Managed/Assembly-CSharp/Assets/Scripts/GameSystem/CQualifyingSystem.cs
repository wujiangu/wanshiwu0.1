namespace Assets.Scripts.GameSystem
{
    ...
    public class CQualifyingSystem : Singleton<CQualifyingSystem>
    {
        ...
        private void Qualifying_MenuSelect(CUIEvent uiEvent)
        {
            ...
            {
                if (Singleton<CRoleInfoManager>.GetInstance().GetMasterRoleInfo().m_gradeOfRank == 0)
                {
                    Singleton<CUIManager>.GetInstance().OpenTips("You have not rank info", false, 1f, null, new object[0]);
                }
                else
                {
        ...
        public void RefreshRankInfo()
        {
            ...
            if (form != null)
            {
                ...
                ResRankGradeConf dataByKey = GameDataMgr.rankGradeDatabin.GetDataByKey(masterRoleInfo.m_gradeOfRank);
