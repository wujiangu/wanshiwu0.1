namespace Assets.Scripts.GameSystem
{
    ...
    public class CUICommonSystem : Singleton<CUICommonSystem>
    {
        ...
        public static string GetPvpHeroSkillDesc(int skillId, uint heroId)
        {
            int pureBalanceLevel = CBattleValueAdjust.GetPureBalanceLevel();
            CBattleValueAdjust.SetHeroBalanceValueArr(heroId, ref s_heroValArr, pureBalanceLevel);
            return GetSkillDesc(skillId, s_heroValArr, 1);
        }
        ...
        public static string GetSkillDesc(int skillId, CHeroInfo heroInfo, int skillLevel)
        {
            ...
            if (escapeString != null)
            {
                ...
                {
                    int num2 = CSkillData.CalcEscapeValue(escapeString[i], heroInfo, skillLevel);
        ...
        public static string GetSkillDesc(int skillId, ValueDataInfo[] valueData, int skillLevel)
        {
            ...
            if (escapeString != null)
            {
                for (int i = 0; i < escapeString.Length; i++)
                {
                    int num2 = CSkillData.CalcEscapeValue(escapeString[i], valueData, skillLevel);
                    string newValue = string.Empty;
                    if (num2 != 0)
                    {
                        newValue = num2.ToString();
        ...
        public static string GetSkillDesc(string skillDesc, ValueDataInfo[] valueData, int skillLevel)
        {
            ...
            if (escapeString != null)
            {
                for (int i = 0; i < escapeString.Length; i++)
                {
                    str = str.Replace("[" + escapeString[i] + "]", CSkillData.CalcEscapeValue(escapeString[i], valueData, skillLevel).ToString());
                }
            }
            return str;
        }
        ...
        public static string GetSkillDescLobby(string skillDesc, uint heroId)
        {
            ...
            return GetSkillDesc(skillDesc, s_heroValArr, 1);
        }
        ...
        public static void GetTimeUtilOpen(ResRewardMatchTimeInfo inMatchTimeInfo, out uint utilOpenSec, out int utilOpenDay)
        {
            ...
            DateTime dateTimeNow = Utility.ToUtcTime2Local((long) Singleton<CRoleInfoManager>.GetInstance().GetMasterRoleInfo().getCurrentTimeSinceLogin());
            if (info != null)
            {
                ...
                switch (((RES_CYCLETIME_TYPE) info.bCycleType))
                {
                    ...
                    {
                        int length = info.CycleParm.Length;
                        secUtilOpenOfDay = -1;
                        while (secUtilOpenOfDay < 0)
                        {
        ...
        public static bool IsMatchOpened(ResRewardMatchTimeInfo inMatchTimeInfo)
        {
            ...
            if (info != null)
            {
                ...
                DateTime time3 = Utility.ToUtcTime2Local((long) Singleton<CRoleInfoManager>.instance.GetMasterRoleInfo().getCurrentTimeSinceLogin());
        ...
        private static void OnLoadNewHeroOrSkin3DModel(CUIEvent uiEvent)
        {
            ...
            if (srcFormScript.gameObject.activeSelf)
            {
                ...
                s_last3DModelPath = GetHeroPrefabPath(heroId, (int) tag, true).ObjectName;
        ...
        public static void SetGearItemCell(CUIFormScript formScript, GameObject itemCell, uint gearId, int level)
        {
            if (itemCell != null)
            {
                ...
                if (dataByKey != null)
                {
                    Image component = itemCell.GetComponent<Image>();
                    string prefabPath = string.Format("{0}heroControl_slotBg{1}", "UGUI/Sprite/Common/", dataByKey.bGrade);
                    CUIUtility.SetImageSprite(component, prefabPath, formScript, true, false, false);
                    Image image = itemCell.transform.Find("imgIcon").GetComponent<Image>();
                    prefabPath = string.Format("{0}{1}", CUIUtility.s_Sprite_Dynamic_Icon_Dir, dataByKey.dwIcon);
                    CUIUtility.SetImageSprite(image, prefabPath, formScript, true, false, false);
                    Text text = itemCell.transform.Find("lvlText").GetComponent<Text>();
                    string format = Singleton<CTextManager>.GetInstance().GetText("Common_Level_Format");
                    text.text = (level <= 0) ? string.Empty : string.Format(format, level);
                    itemCell.transform.Find("nameText").GetComponent<Text>().text = StringHelper.UTF8BytesToString(ref dataByKey.szName);
                }
            }
        }
        ...
        public static void SetHeroItemData(CUIFormScript formScript, GameObject item, IHeroData data, enHeroHeadType headType = 0, bool bIconGray = false, enHeroInfoShowType shoeType = 0)
        {
        ...
        public static void SetHeroSkinLabelPic(CUIFormScript formScript, GameObject labelObj, uint heroId, uint skinId)
        {
            if (null != labelObj)
            {
                ...
                if (heroSkin != null)
                {
                    ...
                    if (heroSkin.bIsLimitSkin > 0)
                    {
                        if (!string.IsNullOrEmpty(heroSkin.szLimitLabelPic))
                        {
                            szLimitLabelPic = heroSkin.szLimitLabelPic;
                        }
                        transform.sizeDelta = new Vector2(120f, 38f);
                    }
                    else
                    {
                        ResSkinQualityPicInfo dataByKey = GameDataMgr.skinQualityPicDatabin.GetDataByKey(heroSkin.bSkinQuality);
        ...
        public static void SetHeroSkinQualityPic(CUIFormScript formScript, GameObject qualityObj, uint heroId, uint skinId)
        {
            if (null != qualityObj)
            {
                ...
                if (heroSkin != null)
                {
                    if (heroSkin.bIsLimitSkin > 0)
                    {
                        string prefabPath = string.Format("{0}{1}", CUIUtility.s_Sprite_Dynamic_SkinQuality_Dir, StringHelper.UTF8BytesToString(ref heroSkin.szLimitQualityPic));
