namespace Assets.Scripts.GameSystem
{
    ...
    public class NobeSys : MonoSingleton<NobeSys>
    {
        ...
        public static void OnGetNobeVipInfo(CSPkg msg)
        {
            ...
            else if (MonoSingleton<NobeSys>.GetInstance().m_IsShowDelayNobeTips && (msg.stPkgData.stGameVipNtf.dwNtfType > 0))
            {
        ...
        public void SetGameCenterVisible(GameObject obj, COM_PRIVILEGE_TYPE privilegeType, ApolloPlatform platform, bool isHideInIos = false, bool isIosBlock = false)
        {
            if (obj != null)
            {
                bool bActive = (((privilegeType == COM_PRIVILEGE_TYPE.COM_PRIVILEGE_TYPE_WXGAMECENTER_LOGIN) && !isIosBlock) && (Singleton<ApolloHelper>.GetInstance().CurPlatform == ApolloPlatform.Wechat)) && (ApolloPlatform.Wechat == platform);
        ...
        private void SetNobePrivilege(uint curLv, CUIFormScript formScript)
        {
            int num = 1;
            int num2 = 0;
            int num3 = 100;
            ...
            {
                Transform transform = formScript.m_formWidgets[3].transform.FindChild(string.Format("TeQuan{0}", m + 1));
                if (transform != null)
                {
                    transform.gameObject.CustomSetActive(true);
                    Transform transform2 = transform.FindChild(string.Format("Icon", new object[0]));
            ...
            for (int n = num; n <= 4; n++)
            {
                Transform transform4 = formScript.m_formWidgets[3].transform.FindChild(string.Format("TeQuan{0}", n));
                if (transform4 != null)
                {
        ...
        private void ShowLoginTips(SCPKG_GAME_VIP_NTF vipInfo)
        {
            if (vipInfo != null)
            {
                switch (vipInfo.dwNtfType)
                {
                    ...
                    case 2:
                        Singleton<CUIManager>.GetInstance().OpenMessageBox(Singleton<CTextManager>.GetInstance().GetText("VIPLOSE"), false);
                        break;
                    ...
                    case 5:
                    {
                        string str2 = string.Format(Singleton<CTextManager>.GetInstance().GetText("VIPDEGRADE"), vipInfo.stGameVipClient.dwCurLevel);
        ...
        protected void ShowNobeLevelUpForm(SCPKG_GAME_VIP_NTF vipInfo)
        {
            ...
            if (formScript != null)
            {
                ...
                this.SetNobePrivilege(dwCurLevel, formScript);
        ...
        private void UpdateBaseInfo()
        {
            if (this.m_vipInfo != null)
            {
                ...
                if (dwCurLevel >= num3)
                {
                    ...
                    ResVIPCoupons dataByKey = GameDataMgr.resVipDianQuan.GetDataByKey(dwCurLevel);
                    if (dataByKey != null)
                    {
                ...
                {
                    int key = dwCurLevel + 1;
                    ResVIPCoupons coupons2 = GameDataMgr.resVipDianQuan.GetDataByKey(key);
                    if (coupons2 != null)
                    {
                        int num5 = ((int) coupons2.dwConsumeCoupons) - dwScore;
                        if (num5 > 0)
                        {
                            str = string.Format("再消费{0}点券，升级到贵族{1}", num5, key);
                            text.gameObject.CustomSetActive(false);
                            this.UpdateImageLevelTips(widget, key, num5);
                        ...
                        else
                        {
                            ...
                            ResVIPCoupons coupons3 = GameDataMgr.resVipDianQuan.GetDataByKey(dwMaxLevel);
                ...
                for (uint k = 0; k < 8; k++)
                {
                    ...
                    if (view4.ContainsKey(k + 1))
                    {
                        ...
                        for (int num13 = 0; num13 < view6.Count; num13++)
                        {
                            ResNobeInfo info4 = view6[num13];
                            Transform transform = elemenet.transform.Find("itemCell" + num13);
                            if (transform != null)
                            {
        ...
        private void UpdateGrowInfo()
        {
            if (this.m_vipInfo != null)
            {
                ...
                {
                    ResVIPCoupons item = GameDataMgr.resVipDianQuan.GetDataByKey(i);
                    int num10 = (i - 1) / 5;
                    if (!view.ContainsKey(num10))
                    {
                        ListView<ResVIPCoupons> view2 = new ListView<ResVIPCoupons>();
                        view2.Add(item);
                        view.Add(num10, view2);
                    }
                    else
                    {
                        view[num10].Add(item);
                ...
                for (int j = 0; j < amount; j++)
                {
                    ...
                    if (view.ContainsKey(j))
                    {
                        ...
                        for (int k = 0; k < view4.Count; k++)
                        {
                            ResVIPCoupons coupons5 = view4[k];
                            GameObject gameObject = elemenet.transform.Find("itemCell" + k).gameObject;
                            gameObject.CustomSetActive(true);
        ...
        private void UpdateImageLevelTips(GameObject parent, int level, int resPoint)
        {
