    ...
    internal class PVESettleView
    {
        ...
        public static void SetRewardFormData(CUIFormScript form, COMDT_SETTLE_RESULT_DETAIL settleData)
        {
            ...
            if (curLvelContext != null)
            {
                ...
                if (curLvelContext.GameType == COM_GAME_TYPE.COM_SINGLE_GAME_OF_ACTIVITY)
                {
                    gameObject.CustomSetActive(false);
                }
                else
                {
                    int levelId = CAdventureSys.GetNextLevelId(curLvelContext.iChapterNo, curLvelContext.bLevelNo, curLvelContext.difficulty);
                    if (levelId != 0)
                    {
                ...
                if (curLvelContext.difficulty == 1)
                {
                    obj4.CustomSetActive(false);
                    obj5.CustomSetActive(false);
                }
                else if (curLvelContext.difficulty == 2)
                {
                    obj3.CustomSetActive(false);
                    obj5.CustomSetActive(false);
                }
                else if (curLvelContext.difficulty == 3)
                {
                    obj4.CustomSetActive(false);
                    obj3.CustomSetActive(false);
                }
                component.text = string.Format(curLvelContext.LevelName, new object[0]);
        ...
        {
            <ShowPlayerLevelUp>c__AnonStorey4F storeyf = new <ShowPlayerLevelUp>c__AnonStorey4F {
                newLvl = newLvl
            };
            if (form != null)
            {
                <ShowPlayerLevelUp>c__AnonStorey50 storey = new <ShowPlayerLevelUp>c__AnonStorey50 {
                    <>f__ref$79 = storeyf
                };
                GameObject gameObject = form.transform.Find("PlayerLvlUp").gameObject;
                gameObject.transform.Find("bg/TxtPlayerLvl").gameObject.GetComponent<Text>().text = storeyf.newLvl.ToString();
                ...
                object[] objArray2 = new object[] { storeyf.newLvl };
                DebugHelper.Assert(GameDataMgr.acntExpDatabin.GetDataByKey((uint) storeyf.newLvl) != null, "Can't find acnt exp config -- level {0}", objArray2);
                ...
                int num2 = CSymbolInfo.GetSymbolPosOpenCnt(storeyf.newLvl);
                storey.hasBuy = false;
                storey.master = Singleton<CRoleInfoManager>.instance.GetMasterRoleInfo();
                if ((storey.master != null) && (symbolPosOpenCnt < num2))
                {
                    GameDataMgr.symbolPosDatabin.Accept(new Action<ResSymbolPos>(storey.<>m__58));
                }
                transform.gameObject.CustomSetActive(!storey.hasBuy && (num2 > symbolPosOpenCnt));
                if (!storey.hasBuy && (num2 > symbolPosOpenCnt))
                {
                ...
                int num4 = CSymbolInfo.GetSymbolLvlLimit(storeyf.newLvl);
                ...
                ResHeroSymbolLvl dataByKey = GameDataMgr.heroSymbolLvlDatabin.GetDataByKey(storeyf.newLvl);
                if (dataByKey != null)
                {
        ...
        private static void ShowReward(CUIFormScript belongForm, COMDT_SETTLE_RESULT_DETAIL settleData)
        {
            if (Singleton<CRoleInfoManager>.GetInstance().GetMasterRoleInfo() != null)
            {
                ...
                if (stAcntInfo != null)
                {
                    ...
                    ResAcntPvpExpInfo dataByKey = GameDataMgr.acntPvpExpDatabin.GetDataByKey((byte) stAcntInfo.dwPvpLv);
        ...
        {
            if (rewardForm.transform.Find("Root/EscapeAnim").gameObject.activeSelf)
            {
        ...
        [CompilerGenerated]
        private sealed class <ShowPlayerLevelUp>c__AnonStorey4F
        {
            internal int newLvl;
        }

        [CompilerGenerated]
        private sealed class <ShowPlayerLevelUp>c__AnonStorey50
        {
            internal PVESettleView.<ShowPlayerLevelUp>c__AnonStorey4F <>f__ref$79;
            internal bool hasBuy;
            internal CRoleInfo master;

            internal void <>m__58(ResSymbolPos rule)
            {
                if ((rule != null) && (rule.wOpenLevel == this.<>f__ref$79.newLvl))
                {
