namespace Assets.Scripts.GameSystem
{
    ...
    public class CArenaSystem : Singleton<CArenaSystem>
    {
        ...
        private void Arena_BuyChangeTimes(CUIEvent uiEvent)
        {
            ...
            ResShopInfo dataByKey = GameDataMgr.resShopInfoDatabin.GetDataByKey((int) ((ushort) shopInfoCfgId));
            if (dataByKey != null)
            {
                ...
                CMallSystem.TryToPay(enPayPurpose.Buy, Singleton<CTextManager>.GetInstance().GetText("Arena_Arena_Count_Buy", args), payType, dwCoinPrice, enUIEventID.Arena_ConfirmBuyChangeTimes, ref uiEvent.m_eventParams, enUIEventID.None, true, true);
            }
        }
        ...
        private void Arena_OpenForm(CUIEvent uiEvent)
        {
            if (Singleton<CFunctionUnlockSys>.instance.FucIsUnlock(RES_SPECIALFUNCUNLOCK_TYPE.RES_SPECIALFUNCUNLOCKTYPE_ARENA))
            {
                ...
                {
                    MonoSingleton<NewbieGuideManager>.GetInstance().SetNewbieBit(15, true);
                    this.Reset();
                    SendGetFightHeroListMSG();
                }
            }
        ...
        private void Arena_OpenPlayerInfoForm(CUIEvent uiEvent)
        {
            ...
            {
                Singleton<CUIManager>.GetInstance().OpenTips(Singleton<CTextManager>.GetInstance().GetText("Arena_ARENAADDMEM_ERR_BATTLELISTISNULL"), false, 1f, null, new object[0]);
                Singleton<CUIManager>.GetInstance().CloseForm(s_arenaPlayerInfoFormPath);
            }
            else
            {
                int num = this.m_nextCanFightTimes - Singleton<CRoleInfoManager>.GetInstance().GetMasterRoleInfo().GetElapseSecondsSinceLogin();
                if (num > 0)
                {
                    Singleton<CUIManager>.GetInstance().OpenTips(Singleton<CTextManager>.GetInstance().GetText("Arena_ERR_LIMIT_CD"), false, 1f, null, new object[0]);
                    Singleton<CUIManager>.GetInstance().CloseForm(s_arenaPlayerInfoFormPath);
                }
                else if (this.m_serverInfo.chAlreadyFightCnt >= GameDataMgr.globalInfoDatabin.GetDataByKey((uint) 0x30).dwConfValue)
                {
                    Singleton<CUIManager>.GetInstance().OpenTips(Singleton<CTextManager>.GetInstance().GetText("Arena_ERR_LIMIT_CNT"), false, 1f, null, new object[0]);
                    Singleton<CUIManager>.GetInstance().CloseForm(s_arenaPlayerInfoFormPath);
                }
                else
                {
                    ...
                    for (int i = 0; i < script2.GetElementAmount(); i++)
                    {
                        ...
                        if (comdt_arena_herodetail.dwHeroId != 0)
                        {
                            ...
                            CUICommonSystem.SetHeroItemData(formScript, item, data, enHeroHeadType.enIcon, false, enHeroInfoShowType.enPVE);
                        }
                        else
                        {
        ...
        {
            if (((Singleton<CRoleInfoManager>.GetInstance().GetMasterRoleInfo().GetElapseSecondsSinceLogin() - this.m_lastRecordRequestTime) > 60L) || (this.m_lastRecordRequestTime == 0))
            {
                SendGetRecordMSG();
            }
            else
        ...
        private void Arena_RankElementEnable(CUIEvent uiEvent)
        {
            int srcWidgetIndexInBelongedList = uiEvent.m_srcWidgetIndexInBelongedList;
            ...
            COMDT_ARENA_MEMBER_OF_ACNT fighterInfo = GetFighterInfo(this.m_rankInfoList.astFigterDetail[srcWidgetIndexInBelongedList].stFigterData);
            ...
            for (int i = 0; i < objArray.Length; i++)
            {
                if (i < fighterInfo.stBattleHero.astHero.Length)
                {
                    ...
                    if (comdt_arena_herodetail.dwHeroId != 0)
                    {
                        ...
                        data.m_quaility = comdt_arena_herodetail.stHeroQuality.wQuality;
                        data.m_subQualility = comdt_arena_herodetail.stHeroQuality.wSubQuality;
                        CUICommonSystem.SetHeroItemData(uiEvent.m_srcFormScript, objArray[i], data, enHeroHeadType.enIcon, false, enHeroInfoShowType.enPVE);
                    }
                    else
                    {
        ...
        private void Arena_RankInfoMenuClick(CUIEvent uiEvent)
        {
            ...
            switch (selectedIndex)
            {
                case 0:
                    ...
                    gameObject.transform.Find("List").gameObject.GetComponent<CUIListScript>().SetElementAmount(this.m_rankInfoList.bFigterNum);
                    break;
        ...
        private void Arena_ResetByNewDay(CUIEvent uiEvent)
        {
            ...
            this.m_serverInfo.dwNextRefreshLeftTime = ((uint) Singleton<CRoleInfoManager>.GetInstance().GetMasterRoleInfo().GetElapseSecondsSinceLogin()) + Utility.s_daySecond;
            this.m_serverInfo.dwLeftFightCoolTime = 0;
            this.m_serverInfo.chAlreadyFightCnt = 0;
            this.m_nextCanFightTimes = Singleton<CRoleInfoManager>.GetInstance().GetMasterRoleInfo().GetElapseSecondsSinceLogin();
            this.RefreshArenaForm();
        }
        ...
        {
            uint payValue = 10;
            int num2 = this.m_nextCanFightTimes - Singleton<CRoleInfoManager>.GetInstance().GetMasterRoleInfo().GetElapseSecondsSinceLogin();
            if (num2 > 0)
            {
                ResClrCD dataByKey = GameDataMgr.cdDatabin.GetDataByKey(1);
                ...
                CMallSystem.TryToPay(enPayPurpose.Buy, Singleton<CTextManager>.GetInstance().GetText("Arena_Cd_End"), payType, payValue, enUIEventID.Arena_ConfirmBuyResetCD, ref uiEvent.m_eventParams, enUIEventID.None, true, true);
            }
        }
        ...
        private void Arena_StartFight(CUIEvent uiEvent)
        {
            ...
            ResLevelCfgInfo dataByKey = GameDataMgr.arenaLevelDatabin.GetDataByKey(s_mapKey);
            DebugHelper.Assert(dataByKey != null);
            byte iHeroNum = (byte) dataByKey.iHeroNum;
            uint dwBattleListID = dataByKey.dwBattleListID;
            Singleton<CHeroSelectSystem>.GetInstance().SetPveDataWithArena(iHeroNum, dwBattleListID, reportInfo, Singleton<CTextManager>.GetInstance().GetText("Arena_Arena"));
            stUIEventParams par = new stUIEventParams {
                heroSelectGameType = enSelectHeroType.enArena
            };
        ...
        {
            Singleton<CHeroSelectSystem>.GetInstance().SetPveDataWithArena(3, 0, null, Singleton<CTextManager>.GetInstance().GetText("Arena_Arena_Defensive_Team"));
            stUIEventParams par = new stUIEventParams {
                heroSelectGameType = enSelectHeroType.enArenaDefTeamConfig,
                heroIdList = new List<uint>(Singleton<CRoleInfoManager>.GetInstance().GetMasterRoleInfo().m_arenaDefHeroList)
            };
            Singleton<CUIEventManager>.GetInstance().DispatchUIEvent(enUIEventID.HeroSelect_OpenForm, par);
        }
        ...
        public void BattleReturn(bool isWin)
        {
            if (!isWin)
            {
                Singleton<CArenaSystem>.GetInstance().m_nextCanFightTimes = Singleton<CRoleInfoManager>.GetInstance().GetMasterRoleInfo().GetElapseSecondsSinceLogin() + ((int) GameDataMgr.globalInfoDatabin.GetDataByKey((uint) 0x31).dwConfValue);
            }
        ...
        {
            <GetRewardUseableList>c__AnonStorey32 storey = new <GetRewardUseableList>c__AnonStorey32 {
                rank = rank
            };
            ListView<CUseable> view = new ListView<CUseable>();
            ResArenaOneDayReward reward = GameDataMgr.arenaRewardDatabin.FindIf(new Func<ResArenaOneDayReward, bool>(storey, (IntPtr) this.<>m__22));
            if (reward != null)
            {
        ...
        {
            <GetRobotHeroInfo>c__AnonStorey33 storey = new <GetRobotHeroInfo>c__AnonStorey33 {
                forceValue = forceValue,
                res = null
            };
            GameDataMgr.robotHeroInfo.Accept(new Action<ResRobotPower>(storey.<>m__23));
            if (storey.res == null)
            {
                GameDataMgr.robotHeroInfo.Accept(new Action<ResRobotPower>(storey.<>m__24));
            }
            return storey.res;
        }
        ...
        public override void Init()
        {
            ...
            Singleton<CUIEventManager>.GetInstance().AddUIEventListener(enUIEventID.Arena_RankElementEnable, new CUIEventManager.OnUIEventHandler(this.Arena_RankElementEnable));
        ...
        public void InitServerData(CSDT_ACNT_ARENADATA serverData)
        {
            ...
            this.m_nextCanFightTimes = Singleton<CRoleInfoManager>.GetInstance().GetMasterRoleInfo().GetElapseSecondsSinceLogin() + ((int) this.m_serverInfo.dwLeftFightCoolTime);
        }
        ...
        public static void ReciveDefTeamListInfo(CSPkg msg)
        {
            ...
            {
                Singleton<CUIManager>.GetInstance().OpenTips(Utility.ProtErrCodeToStr(0xb55, stSetBattleListOfArenaRsp.bErrCode), false, 1f, null, new object[0]);
            }
        ...
        public static void ReciveJoinArenaInfo(CSPkg msg)
        {
            ...
            {
                Singleton<CUIManager>.GetInstance().OpenTips(Utility.ProtErrCodeToStr(0xb58, stJoinArenaRsp.stResult.bErrCode), false, 1f, null, new object[0]);
            }
            Singleton<CUIManager>.GetInstance().CloseSendMsgAlert();
        }
        ...
        public static void ReciveRankListInfo(CSPkg msg)
        {
            ...
            Singleton<CUIManager>.GetInstance().CloseSendMsgAlert();
            if (Singleton<CUIManager>.GetInstance().GetForm(s_arenaRankInfoFormPath) == null)
            {
        ...
        public static void ReciveRecordListInfo(CSPkg msg)
        {
            ...
            Singleton<CArenaSystem>.GetInstance().m_lastRecordRequestTime = (uint) Singleton<CRoleInfoManager>.GetInstance().GetMasterRoleInfo().GetElapseSecondsSinceLogin();
            Singleton<CUIManager>.GetInstance().CloseSendMsgAlert();
            Singleton<CUIManager>.GetInstance().OpenForm(s_arenaFightRecordFormPath, false, true);
            Singleton<CArenaSystem>.GetInstance().RefreshRecordForm();
        }
        ...
        public static void ReciveResetCD(CSPkg msg)
        {
            ...
            {
                Singleton<CArenaSystem>.GetInstance().m_nextCanFightTimes = Singleton<CRoleInfoManager>.GetInstance().GetMasterRoleInfo().GetElapseSecondsSinceLogin();
                Singleton<CArenaSystem>.GetInstance().RefreshArenaForm();
            }
            else
            {
                Singleton<CUIManager>.GetInstance().OpenTips("Reset CD Fail", false, 1f, null, new object[0]);
            }
            Singleton<CUIManager>.GetInstance().CloseSendMsgAlert();
        }
        ...
        public void RefreshArenaForm()
        {
            ...
            if (form != null)
            {
                ...
                component.SetTotalTime((float) (this.m_serverInfo.dwNextRefreshLeftTime - Singleton<CRoleInfoManager>.GetInstance().GetMasterRoleInfo().GetElapseSecondsSinceLogin()));
                ...
                for (int i = 0; i < arenaDefHeroList.Count; i++)
                {
                    ...
                    CUICommonSystem.SetHeroItemData(form, item, data, enHeroHeadType.enIcon, false, enHeroInfoShowType.enPVE);
                }
                ...
                int num4 = this.m_nextCanFightTimes - Singleton<CRoleInfoManager>.GetInstance().GetMasterRoleInfo().GetElapseSecondsSinceLogin();
                if (num4 <= 0)
                {
                    gameObject.CustomSetActive(false);
                    script6.EndTimer();
        ...
        public static void SendGetFightHeroListMSG()
        {
            CSPkg msg = NetworkModule.CreateDefaultCSPKG(0xb5b);
            Singleton<NetworkModule>.GetInstance().SendLobbyMsg(ref msg, true);
        }

        public static void SendGetRankListMSG()
        {
            CSPkg msg = NetworkModule.CreateDefaultCSPKG(0xb5d);
            Singleton<NetworkModule>.GetInstance().SendLobbyMsg(ref msg, true);
        }

        public static void SendGetRecordMSG()
        {
        ...
        [CompilerGenerated]
        private sealed class <GetRewardUseableList>c__AnonStorey32
        {
            internal int rank;

            internal bool <>m__22(ResArenaOneDayReward x)
            {
        ...
        [CompilerGenerated]
        private sealed class <GetRobotHeroInfo>c__AnonStorey33
        {
            internal uint forceValue;
            internal ResRobotPower res;

            internal void <>m__23(ResRobotPower x)
            {
            ...
            internal void <>m__24(ResRobotPower x)
            {
                if ((x.dwPower < this.forceValue) && ((this.res == null) || (this.res.dwPower < x.dwPower)))
                {
                    this.res = x;
