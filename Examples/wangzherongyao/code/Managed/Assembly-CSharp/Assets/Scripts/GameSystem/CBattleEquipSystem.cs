    ...
    public class CBattleEquipSystem
    {
        ...
        private void BuyEquip(ushort equipID, ref PoolObjHandle<ActorRoot> actor)
        {
            if ((this.m_isEnabled && (actor != 0)) && actor.handle.EquipComponent.IsPermitedToBuyEquip(this.m_isInEquipLimitedLevel))
            {
                ...
                {
                    actor.handle.ValueComponent.ChangeGoldCoinInBattle((int) (freeEquipBuyPrice.m_buyPrice * -1), false, false, new Vector3());
        ...
        private void ClearAllEquipSelect()
        {
            if (null != this.m_equipFormScript)
            {
                ...
                this.ClearEquipLevelPanelSelect(widget.transform);
                GameObject obj3 = this.m_equipFormScript.GetWidget(2);
                this.ClearEquipLevelPanelSelect(obj3.transform);
                GameObject obj4 = this.m_equipFormScript.GetWidget(3);
                this.ClearEquipLevelPanelSelect(obj4.transform);
                GameObject obj5 = this.m_equipFormScript.GetWidget(4);
                this.ClearEquipBagPanelSelect(obj5.transform);
        ...
        public void ClearEquipBagPanelSelect(Transform bagPanel)
        {
            if (null != bagPanel)
            {
                for (int i = 0; i < 6; i++)
                {
                    Transform equipItemObj = bagPanel.Find(string.Format("equipItem{0}", i));
                    this.SetEquipItemSelectFlag(equipItemObj, false);
                }
            }
        }
        ...
        private void ClearEquipLevelPanelSelect(Transform equipPanel)
        {
            if (null != equipPanel)
            {
                for (int i = 0; i < 8; i++)
                {
                    Transform equipItemObj = equipPanel.Find(string.Format("equipItem{0}", i));
                    this.SetEquipItemSelectFlag(equipItemObj, false);
                }
            }
        }
        ...
        public void HeroEquipChanged(uint actorObjectID, stEquipInfo[] equips)
        {
        ...
        private void InitEquipLevelPanel(Transform equipPanel)
        {
            if (null != equipPanel)
            {
                int num = 0;
                for (num = 0; num < 8; num++)
                {
                    Transform transform = equipPanel.Find(string.Format("equipItem{0}", num));
                    if (transform != null)
                    {
                        CanvasGroup component = transform.GetComponent<CanvasGroup>();
                        if (component == null)
                        {
                            component = transform.gameObject.AddComponent<CanvasGroup>();
                        }
                        component.alpha = 1f;
                    }
                }
        ...
        public void OnEquipFormOpen(CUIEvent uiEvent)
        {
            ...
            if (this.m_equipFormScript != null)
            {
                ...
                GameObject widget = this.m_equipFormScript.GetWidget(1);
                this.InitEquipLevelPanel(widget.transform);
                GameObject obj5 = this.m_equipFormScript.GetWidget(2);
                this.InitEquipLevelPanel(obj5.transform);
                GameObject obj6 = this.m_equipFormScript.GetWidget(3);
                this.InitEquipLevelPanel(obj6.transform);
                this.ClearAllEquipSelect();
                this.CloseEquipFormRightPanel();
                GameObject listObj = this.m_equipFormScript.GetWidget(0);
                CTextManager instance = Singleton<CTextManager>.GetInstance();
                string[] titleList = new string[] { instance.GetText("Equip_Usage_Recommend"), instance.GetText("Equip_Usage_PhyAttack"), instance.GetText("Equip_Usage_MagicAttack"), instance.GetText("Equip_Usage_Defence"), instance.GetText("Equip_Usage_Move"), instance.GetText("Equip_Usage_Jungle") };
                CUICommonSystem.InitMenuPanel(listObj, titleList, (int) this.m_curEquipUsage);
            }
        ...
        public void OnHeroRecommendEquipInit(uint actorObjectID)
        {
            if (this.m_isEnabled && ((this.m_hostCtrlHero != 0) && (actorObjectID == this.m_hostCtrlHero.handle.ObjID)))
            {
        ...
        public void RefreshEquipBagPanel(Transform bagPanel)
        {
            if ((null != bagPanel) && (this.m_hostCtrlHero != 0))
            {
                ...
                for (int i = 0; i < 6; i++)
                {
                    Transform transform = bagPanel.Find(string.Format("equipItem{0}", i));
                    ...
                    if (equips[i].m_amount >= 1)
                    {
                        ...
                        if (equipInfo != null)
                        {
                            ...
                            eventParams.battleEquipPar.equipItemObj = transform;
        ...
        public void RefreshEquipBuyPanel(Transform buyPanel)
        {
            ...
            {
                CEquipBuyPrice freeEquipBuyPrice = this.GetFreeEquipBuyPrice();
                bool flag = this.IsEquipCanBought(this.m_selEquipInfo.m_equipID, ref this.m_hostCtrlHero, ref freeEquipBuyPrice);
        ...
        public void RefreshEquipInfoPanel(Transform infoPanel)
        {
            if ((null != infoPanel) && (this.m_selEquipInfo != null))
            {
                ...
                infoPanel.Find("equipPropertyDescText").GetComponent<Text>().text = this.m_selEquipInfo.m_equipPropertyDesc;
            }
        }
        ...
        private void RefreshEquipItem(Transform equipItem, ushort equipID)
        {
            if (null != equipItem)
            {
                ...
                else if (equipInfo.m_resEquipInBattle != null)
                {
                    ...
                    component.SetSprite(equipInfo.m_equipIconPath, this.m_equipFormScript, true, false, false);
                    CEquipBuyPrice freeEquipBuyPrice = this.GetFreeEquipBuyPrice();
                    bool flag = this.IsEquipCanBought(equipID, ref this.m_hostCtrlHero, ref freeEquipBuyPrice);
                    component.color = (!flag && !this.m_hostCtrlHero.handle.EquipComponent.HasEquip(equipID, 1)) ? CUIUtility.s_Color_GrayShader : CUIUtility.s_Color_White;
                    equipItem.Find("nameText").GetComponent<Text>().text = equipInfo.m_equipName;
                    equipItem.Find("priceText").GetComponent<Text>().text = freeEquipBuyPrice.m_buyPrice.ToUintString();
                    bool bActive = this.m_hostCtrlHero._handleObj.EquipComponent.HasEquip(equipInfo.m_equipID, 1);
        ...
        private void RefreshEquipLevelPanel(Transform equipPanel, int level)
        {
            if (null != equipPanel)
            {
                ...
                while ((num2 < 8) && (num2 < count))
                {
                    ...
                    equipItem.GetComponent<CanvasGroup>().alpha = 1f;
                    num2++;
                }
                while (num2 < 8)
                {
                    equipPanel.Find(string.Format("equipItem{0}", num2)).GetComponent<CanvasGroup>().alpha = 0f;
                    num2++;
                }
            }
        }
        ...
        private void RefreshHostPlayerQuicklyBuyEquipPanel(bool hostCtrlHeroPermitedToBuyEquip)
        {
            ...
            if (formScript != null)
            {
                GameObject widget = formScript.GetWidget(0x3a);
                if (widget != null)
                {
                ...
                GameObject obj3 = formScript.GetWidget(0x3b);
                if (obj3 != null)
                {
                ...
                for (int i = 0; i < this.m_hostPlayerQuicklyBuyEquipIDs.Length; i++)
                {
                    ...
                    if (widget != null)
                    {
                        ...
                        if (equipInfo != null)
                        {
                            ...
                            widget.transform.Find("imgIcon").GetComponent<Image>().SetSprite(equipInfo.m_equipIconPath, this.m_battleFormScript, true, false, false);
                            widget.transform.Find("Panel_Info/descText").GetComponent<Text>().text = string.Format("<color=#ffa500>{0}</color>\n{1}", equipInfo.m_equipName, equipInfo.m_equipDesc);
                            widget.transform.Find("moneyText").GetComponent<Text>().text = ((uint) this.GetHostPlayerCachedEquipBuyPrice(equipInfo.m_equipID).m_buyPrice).ToString();
        ...
        private void SellEquip(int equipIndexInGrid, ref PoolObjHandle<ActorRoot> actor)
        {
            if ((this.m_isEnabled && (equipIndexInGrid >= 0)) && (((equipIndexInGrid < 6) && (actor != 0)) && actor.handle.EquipComponent.IsPermitedToBuyEquip(this.m_isInEquipLimitedLevel)))
            {
                ...
                {
                    ResEquipInBattle dataByKey = GameDataMgr.m_equipInBattleDatabin.GetDataByKey((int) equips[equipIndexInGrid].m_equipID);
                    if (dataByKey != null)
                    {
                        actor.handle.EquipComponent.RemoveEquip(equipIndexInGrid);
                        actor.handle.ValueComponent.ChangeGoldCoinInBattle((int) dataByKey.dwSalePrice, false, false, new Vector3());
                    }
                }
            }
        }
