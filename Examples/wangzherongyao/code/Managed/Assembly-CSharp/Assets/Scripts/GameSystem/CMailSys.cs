namespace Assets.Scripts.GameSystem
{
    ...
    public class CMailSys : Singleton<CMailSys>
    {
        ...
        private void OnAddToMsgCenter(CUIEvent uiEvent)
        {
            ...
            if (comdt_friend_info != null)
            {
                this.AddFriendInviteMail(comdt_friend_info.stUin.ullUid, (uint) masterRoleInfo.getCurrentTimeSinceLogin());
            }
        }
        ...
        private void OnCloseMailForm(CUIEvent uiEvent)
        {
            this.m_friAccessAll = false;
            this.m_sysAccessAll = false;
            Singleton<CUIManager>.GetInstance().CloseForm(MAIL_FORM_PATH);
            this.m_mailView.Form = null;
        }
        ...
        public void OnMailGetAccess(CSPkg msg)
        {
            switch (msg.stPkgData.stMailOptRes.stOptInfo.stGetAccess.bResult)
            {
                ...
                case 4:
                    Singleton<CUIManager>.instance.OpenTips(Singleton<CTextManager>.GetInstance().GetText("MailOp_PackageClean"), false, 1f, null, new object[0]);
                    this.m_sysAccessAll = false;
                    goto Label_03C9;

                case 5:
                    Singleton<CUIManager>.instance.OpenTips(Singleton<CTextManager>.GetInstance().GetText("MailOp_Packagefull"), false, 1f, null, new object[0]);
                    this.m_sysAccessAll = false;
                    goto Label_03C9;
                ...
                case 8:
                    if (!this.m_friAccessAll)
                    {
                        Singleton<CUIManager>.instance.OpenTips(Singleton<CTextManager>.GetInstance().GetText("Mailop_GetHeartLimit"), false, 1f, null, new object[0]);
                        Singleton<CUIManager>.GetInstance().CloseSendMsgAlert();
                    }
                    else
                    {
        ...
        public void OnMailSendMailRes(CSPkg msg)
        {
            ...
            switch (msg.stPkgData.stMailOptRes.stOptInfo.stSendMail.bResult)
            {
                ...
                case 6:
                    Singleton<CUIManager>.instance.OpenTips(Singleton<CTextManager>.GetInstance().GetText("MailOp_ListGeting"), false, 1f, null, new object[0]);
                    return;
        ...
        private void OnOpenMailForm(CUIEvent uiEvent)
        {
            this.m_friAccessAll = false;
            this.m_sysAccessAll = false;
            CUIFormScript script = Singleton<CUIManager>.GetInstance().OpenForm(MAIL_FORM_PATH, false, true);
            this.m_mailView.Form = script;
        ...
        private void OnSystemMailDelete(CUIEvent uiEvent)
        {
            if ((this.m_sysMailList != null) && (this.m_sysMailList.Count > 0))
            {
                ...
                {
                    Singleton<CUIManager>.instance.OpenTips(Singleton<CTextManager>.GetInstance().GetText(canBeDeleted.ToString()), false, 1f, null, new object[0]);
                }
            }
        }
