namespace Assets.Scripts.GameSystem
{
    ...
    public class CCheatSystem : Singleton<CCheatSystem>
    {
        ...
        private static string[] s_TDirServerTypeName = new string[] { "使用默认", "测试服", "中转服", "正式服", "体验版正式服", "体验版中转服", "测试专用服" };
        private static string[] s_tversionServerTypeName = new string[] { "正式服", "中转服", "测试服", "体验版正式服", "体验版中转服", "测试专用服", "跳过更新" };
        ...
        private void OnClearCache(CUIEvent uiEvent)
        {
            ...
            {
                Singleton<CUIManager>.GetInstance().OpenTips("缓存清理成功！", false, 1f, null, new object[0]);
            }
            else
            {
                Singleton<CUIManager>.GetInstance().OpenTips("缓存清理失败！", false, 1f, null, new object[0]);
            }
        }
