namespace Assets.Scripts.GameSystem
{
    ...
    public class CMallSymbolGiftController : Singleton<CMallSymbolGiftController>
    {
        ...
        public ResShopInfo m_CommonDrawSymbolOneTimeCost;
        public byte m_CurrentRewardIdx;
        private bool m_IsDrawing;
        public bool[] m_ItemCellFlag;
        public byte m_SeniorDrawSymbolCycle;
        public ResShopInfo m_SeniorDrawSymbolFiveTimeCost;
        public ResShopInfo m_SeniorDrawSymbolOneTimeCost;
        public ResShopInfo m_SeniorDrawSymbolOneTimeFreeCost;
        public bool m_UIToggle;
        public const int MAX_SHOPDRAW_LIMIT = 0x3e8;
        public bool reqSent;
        public static int reqSentTimerSeq;
        public static int[] SymbolGiftSeniorDrawCntLimit = new int[] { 0, 500, 500, 0x62, 0x31 };
        ...
        public void Draw(CUIFormScript form)
        {
            if (Singleton<CMallSystem>.GetInstance().m_IsMallFormOpen && !this.m_IsDrawing)
            {
                ...
                instance.AddUIEventListener(enUIEventID.Lottery_Common_BuyFiveSymbol, new CUIEventManager.OnUIEventHandler(this.OnLotteryCommonBuySymbol));
                ...
                Singleton<EventRouter>.instance.AddEventHandler(EventID.Mall_Close_Mall, new Action(this, (IntPtr) this.OnMallClose));
        ...
        private stPayInfo GetPayInfo(RES_SHOPBUY_TYPE drawType, RES_SHOPDRAW_SUBTYPE drawSubType)
        {
            stPayInfo info = new stPayInfo();
            ResShopInfo cfgShopInfo = CPurchaseSys.GetCfgShopInfo(drawType, (int) drawSubType);
            DebugHelper.Assert(cfgShopInfo != null, "金币抽符文商城配置表不对");
            if (cfgShopInfo != null)
            {
        ...
        public static int GetTotalDrawedCnt()
        {
            int num = 5;
            int num2 = 0;
            for (int i = 1; i < num; i++)
            {
                switch (((RES_SHOPDRAW_SUBTYPE) i))
                {
                    ...
                    case RES_SHOPDRAW_SUBTYPE.RES_DRAWSUBTYPE_ONE:
                        num2 += SymbolGiftSeniorDrawedCntInfo[i];
                        break;

                    case RES_SHOPDRAW_SUBTYPE.RES_DRAWSUBTYPE_FIVE:
                        num2 += SymbolGiftSeniorDrawedCntInfo[i] * 5;
                        break;

                    case RES_SHOPDRAW_SUBTYPE.RES_DRAWSUBTYPE_TEN:
                        num2 += SymbolGiftSeniorDrawedCntInfo[i] * 10;
                        break;
                }
            }
            return num2;
        ...
        {
            this.m_CommonDrawSymbolOneTimeCost = CPurchaseSys.GetCfgShopInfo(RES_SHOPBUY_TYPE.RES_BUYTYPE_DRAWSYMBOLCOMMON, 3);
            this.m_SeniorDrawSymbolOneTimeCost = CPurchaseSys.GetCfgShopInfo(RES_SHOPBUY_TYPE.RES_BUYTYPE_DRAWSYMBOLSENIOR, 2);
            this.m_SeniorDrawSymbolFiveTimeCost = CPurchaseSys.GetCfgShopInfo(RES_SHOPBUY_TYPE.RES_BUYTYPE_DRAWSYMBOLSENIOR, 3);
            this.m_SeniorDrawSymbolOneTimeFreeCost = CPurchaseSys.GetCfgShopInfo(RES_SHOPBUY_TYPE.RES_BUYTYPE_DRAWSYMBOLSENIOR, 1);
            this.m_SeniorDrawSymbolCycle = 5;
        }
        ...
        {
            Singleton<CMallSystem>.GetInstance().LoadUIPrefab("UGUI/Form/System/Mall/SymbolGift", "pnlSymbolGift", form.GetWidget(3), form);
        }
        ...
        private void OnLotteryBuySymbolConfirm(CUIEvent uiEvent)
        {
            if (this.reqSent)
            {
                Singleton<CUIManager>.GetInstance().OpenTips("请等待上一次抽奖结果服务器的返回", false, 1f, null, new object[0]);
            }
            else
            {
                this.reqSent = true;
        ...
        private void OnLotteryCloseFx(CUIEvent uiEvent)
        {
            ...
            if ((mallForm != null) && Singleton<CMallSystem>.GetInstance().m_IsMallFormOpen)
            {
                ...
                {
                    <>f__am$cache13 = sequence => Singleton<CSoundManager>.GetInstance().PostEvent("UI_buy_chou_close", null);
                }
                Singleton<CTimerManager>.GetInstance().AddTimer(0x3e8, 1, <>f__am$cache13);
        ...
        private void OnLotteryCommonBuySymbol(CUIEvent uiEvent)
        {
        ...
        private void OnLotteryShowReward(CUIEvent uiEvent)
        {
            ...
            if (obj3 != null)
            {
                ...
                {
                    CSymbolItem iconUseable = componetInChild.m_onClickEventParams.iconUseable as CSymbolItem;
                    if (iconUseable != null)
                    {
        ...
        private void OnMallTabChange()
        {
            ...
            {
                CUIEventManager instance = Singleton<CUIEventManager>.GetInstance();
                instance.RemoveUIEventListener(enUIEventID.Lottery_Common_BuyFiveSymbol, new CUIEventManager.OnUIEventHandler(this.OnLotteryCommonBuySymbol));
                ...
                Singleton<EventRouter>.instance.RemoveEventHandler<CSPkg>("ShopBuyDraw", new Action<CSPkg>(CMallSymbolGiftController.ReceiveLotteryRes));
                Singleton<EventRouter>.instance.RemoveEventHandler(EventID.Mall_Close_Mall, new Action(this, (IntPtr) this.OnMallClose));
        ...
        private void RefreshButtonView()
        {
            ...
            if ((mallForm != null) && Singleton<CMallSystem>.GetInstance().m_IsMallFormOpen)
            {
                ...
                if (masterRoleInfo != null)
                {
                    GameObject gameObject = mallForm.transform.Find("pnlBodyBg/pnlSymbolGift/pnlAction/pnlCommonBuy/btnPanel/BuyButton").gameObject;
                    GameObject obj3 = mallForm.transform.Find("pnlBodyBg/pnlSymbolGift/pnlAction/pnlSeniorBuy/btnBuyOne").gameObject;
                    GameObject obj4 = mallForm.transform.Find("pnlBodyBg/pnlSymbolGift/pnlAction/pnlSeniorBuy/btnBuyOneFree").gameObject;
                    GameObject obj5 = mallForm.transform.Find("pnlBodyBg/pnlSymbolGift/pnlAction/pnlSeniorBuy/btnBuyOne/BuyButton").gameObject;
                    GameObject p = mallForm.transform.Find("pnlBodyBg/pnlSymbolGift/pnlAction/pnlSeniorBuy/btnBuyFive/BuyButton").gameObject;
                    GameObject obj7 = mallForm.transform.Find("pnlBodyBg/pnlSymbolGift/pnlAction/pnlSeniorBuy/pnlCd").gameObject;
                    GameObject obj8 = mallForm.transform.Find("pnlBodyBg/pnlSymbolGift/pnlAction/pnlSeniorBuy/btnBuyOneFree/BuyButton").gameObject;
                    ...
                    if (gameObject != null)
                    {
                        ...
                        if (this.m_CommonDrawSymbolOneTimeCost != null)
                        {
                            info2.m_payType = CMallSystem.ResBuyTypeToPayType(this.m_CommonDrawSymbolOneTimeCost.bCoinType);
                            info2.m_payValue = this.m_CommonDrawSymbolOneTimeCost.dwCoinPrice;
                        }
                        CMallSystem.SetPayButton(mallForm, gameObject.transform as RectTransform, info2.m_payType, info2.m_payValue, enUIEventID.Lottery_Common_BuyFiveSymbol, ref eventParams);
                    }
                    if (this.HasFreeDraw(COM_SHOP_DRAW_TYPE.COM_SHOPDRAW_SYMBOLSENIOR))
                    {
                        ...
                        if (obj8 != null)
                        {
                            ...
                            if (this.m_SeniorDrawSymbolOneTimeFreeCost != null)
                            {
                                info3.m_payType = CMallSystem.ResBuyTypeToPayType(this.m_SeniorDrawSymbolOneTimeFreeCost.bCoinType);
                                info3.m_payValue = this.m_SeniorDrawSymbolOneTimeFreeCost.dwCoinPrice;
                            }
                            CMallSystem.SetPayButton(mallForm, obj8.transform as RectTransform, info3.m_payType, info3.m_payValue, enUIEventID.Lottery_Senior_BuyFreeSymbol, ref eventParams);
                        }
                    }
                    else
                    {
                        ...
                        int num = Math.Max(0, masterRoleInfo.m_freeDrawInfo[4].dwLeftFreeDrawCD - masterRoleInfo.getCurrentTimeSinceLogin());
                        ...
                        if (obj5 != null)
                        {
                            stPayInfo info4 = new stPayInfo();
                            if (this.m_SeniorDrawSymbolOneTimeCost != null)
                            {
                                info4.m_payType = CMallSystem.ResBuyTypeToPayType(this.m_SeniorDrawSymbolOneTimeCost.bCoinType);
                                info4.m_payValue = this.m_SeniorDrawSymbolOneTimeCost.dwCoinPrice;
                            }
                            CMallSystem.SetPayButton(mallForm, obj5.transform as RectTransform, info4.m_payType, info4.m_payValue, enUIEventID.Lottery_Senior_BuyOneSymbol, ref eventParams);
                        }
                    }
                    if (p != null)
                    {
                        ...
                        if (this.m_SeniorDrawSymbolOneTimeCost != null)
                        {
                            info5.m_payType = CMallSystem.ResBuyTypeToPayType(this.m_SeniorDrawSymbolFiveTimeCost.bCoinType);
                            info5.m_payValue = this.m_SeniorDrawSymbolFiveTimeCost.dwCoinPrice;
                            float num2 = (this.m_SeniorDrawSymbolFiveTimeCost.dwCoinPrice * 10) / (this.m_SeniorDrawSymbolOneTimeCost.dwCoinPrice * 5);
                            string str = string.Empty;
                            if ((num2 < 10f) && (Math.Abs((float) (num2 % 1f)) < float.Epsilon))
                            {
                                str = string.Format("{0}折", ((int) num2).ToString("D"));
                            }
                            else if (num2 < 10f)
                            {
                                str = string.Format("{0}折", num2.ToString("0.0"));
                            }
                            Text text = Utility.GetComponetInChild<Text>(p, "tag/Text");
                            if ((text != null) && (str != string.Empty))
                            {
                                text.text = str;
                            }
                        }
                        CMallSystem.SetPayButton(mallForm, p.transform as RectTransform, info5.m_payType, info5.m_payValue, enUIEventID.Lottery_Senior_BuyFiveSymbol, ref eventParams);
                    }
                }
            }
        }
        ...
        private void RefreshDesc()
        {
            ...
            if ((mallForm != null) && Singleton<CMallSystem>.GetInstance().m_IsMallFormOpen)
            {
                ...
                if (Singleton<CMallSystem>.GetInstance().CurTab == CMallSystem.Tab.Symbol_Gifts)
                {
                    if (componetInChild != null)
                    {
                        componetInChild.text = Singleton<CTextManager>.GetInstance().GetText("Mall_Symbol_Gifts_Common_Buy_Dsc");
                    }
                    if (text2 != null)
                    {
                        text2.text = string.Format(Singleton<CTextManager>.GetInstance().GetText("Mall_Symbol_Gifts_Senior_Buy_Dsc"), this.m_SeniorDrawSymbolCycle - (GetTotalDrawedCnt() % this.m_SeniorDrawSymbolCycle));
                    }
                }
            }
        }

        public static void ResetDrawedCnt()
        {
            int num = 5;
            for (int i = 1; i < num; i++)
            {
                if (SymbolGiftSeniorDrawedCntInfo[i] > SymbolGiftSeniorDrawCntLimit[i])
                {
                    SymbolGiftSeniorDrawedCntInfo[i] = SymbolGiftSeniorDrawCntLimit[i];
                }
            }
        }
        ...
        public static void ResetFreeDrawCD(COM_SHOP_DRAW_TYPE drawType)
        {
            ...
            if (masterRoleInfo != null)
            {
                ...
                if ((index >= 0) && (index <= masterRoleInfo.m_freeDrawInfo.Length))
                {
                    ...
                    int dwConfValue = (int) GameDataMgr.globalInfoDatabin.GetDataByKey((uint) 0x7b).dwConfValue;
                    masterRoleInfo.m_freeDrawInfo[index].dwLeftFreeDrawCD = masterRoleInfo.getCurrentTimeSinceLogin() + dwConfValue;
        ...
        private void TryToPayForLotterySymbol(RES_SHOPBUY_TYPE lotteryType, RES_SHOPDRAW_SUBTYPE lotterySubType)
        {
            if ((Singleton<CMallSystem>.GetInstance().m_MallForm != null) && Singleton<CMallSystem>.GetInstance().m_IsMallFormOpen)
            {
                ...
                {
                    stPayInfo payInfo = this.GetPayInfo(lotteryType, lotterySubType);
                    ...
                    CMallSystem.TryToPay(enPayPurpose.Lottery, string.Empty, payInfo.m_payType, payInfo.m_payValue, uIEvent.m_eventID, ref uIEvent.m_eventParams, enUIEventID.None, false, true);
                }
            }
        }
