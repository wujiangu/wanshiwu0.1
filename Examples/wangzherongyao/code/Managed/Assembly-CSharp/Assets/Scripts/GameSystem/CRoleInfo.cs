namespace Assets.Scripts.GameSystem
{
    ...
    public class CRoleInfo
    {
        ...
        public List<uint> freeHeroList = new List<uint>();
        ...
        public byte m_gradeOfRank;
        private CrypticInt32 m_headId;
        private string m_HeadUrl;
        public byte m_HighestGradeOfRank;
        ...
        public bool CheckHeroBuyable(uint inHeroId, RES_SHOPBUY_COINTYPE coinType)
        {
            ...
            if (data != null)
            {
                ...
                ResHeroCfgInfo dataByKey = GameDataMgr.heroDatabin.GetDataByKey(inHeroId);
                ...
                {
                    case RES_SHOPBUY_COINTYPE.RES_SHOPBUY_TYPE_COUPONS:
                        if (masterRoleInfo.DianQuan < ((ulong) dataByKey.dwBuyCoupons))
                        {
                            break;
                        }
                        return true;
                    ...
                    case RES_SHOPBUY_COINTYPE.RES_SHOPBUY_TYPE_PVPCOIN:
                        if (masterRoleInfo.GoldCoin < dataByKey.dwBuyCoin)
                        {
                            break;
                        }
                        return true;
                    ...
                    case RES_SHOPBUY_COINTYPE.RES_SHOPBUY_TYPE_BURNINGCOIN:
                        if (masterRoleInfo.BurningCoin < dataByKey.dwBuyBurnCoin)
                        {
                            break;
                        }
                        return true;
                    ...
                    case RES_SHOPBUY_COINTYPE.RES_SHOPBUY_TYPE_ARENACOIN:
                        if (masterRoleInfo.ArenaCoin < dataByKey.dwBuyArenaCoin)
                        {
                            break;
                        }
                        return true;
                    ...
                    case RES_SHOPBUY_COINTYPE.RES_SHOPBUY_TYPE_DIAMOND:
                        if (masterRoleInfo.Diamond < dataByKey.dwBuyDiamond)
                        {
                            break;
                        }
                        return true;
                    ...
                    case RES_SHOPBUY_COINTYPE.RES_SHOPBUY_TYPE_MIXPAY:
                        if ((masterRoleInfo.DianQuan + masterRoleInfo.Diamond) < ((ulong) dataByKey.dwBuyDiamond))
                        {
                            break;
                        }
                        return true;
        ...
        public int GetCurFirstWinRemainingTimeSec()
        {
            ...
            {
                uint num = (uint) this.getCurrentTimeSinceLogin();
                if (this._nextFirstWinAvailableTime >= num)
                {
                    return (int) (this._nextFirstWinAvailableTime - num);
                }
            }
            return -1;
        }

        public int getCurrentTimeSinceLogin()
        {
            int num = (int) (Time.realtimeSinceStartup - this.m_upToLoginSec);
            return (this.m_sysTime + num);
        }

        public int GetElapseSecondsSinceLogin()
        {
            return (int) (Time.realtimeSinceStartup - this.m_upToLoginSec);
        }
        ...
        public int GetExperienceHeroLeftTime(uint heroId)
        {
            ...
            {
                return (((int) info.m_experienceDeadLine) - Singleton<CRoleInfoManager>.GetInstance().GetMasterRoleInfo().getCurrentTimeSinceLogin());
            }
            return 0;
        }
        ...
        public int GetExperienceSkinLeftTime(uint heroId, uint skinId)
        {
            ...
            {
                return (this.heroExperienceSkinDic[skinCfgId] - Singleton<CRoleInfoManager>.GetInstance().GetMasterRoleInfo().getCurrentTimeSinceLogin());
            }
            return 0;
        }
        ...
        {
            return UT.CalcDeltaHorus((uint) this.getCurrentTimeSinceLogin(), value);
        }
        ...
        public static int GetServerSec()
        {
            ...
            return masterRoleInfo.getCurrentTimeSinceLogin();
        }
        ...
        public void InitClientBits(COMDT_NEWCLIENT_BITS bits)
        {
            ...
            for (int i = 0; i < num; i++)
            {
                int index = i / 0x40;
                int num4 = i % 0x40;
                flagArray[i] = (bits.BitsDetail[index] & (((ulong) 1L) << num4)) > 0L;
            }
            this._clientBits = new BitArray(flagArray);
        }
        ...
        public void InitGuidedStateBits(COMDT_NEWBIE_STATUS_BITS bits)
        {
            ...
            this.GuidedStateBits = new BitArray(flagArray);
        }
        ...
        public void InitNewbieAchieveBits(COMDT_CLIENT_BITS bits)
        {
            ...
            this.NewbieAchieveBits = new BitArray(flagArray);
        }
        ...
        {
            return this.freeHeroList.Contains(heroId);
        }
        ...
        public bool IsFreeHeroAndSelfDontHave(uint heroId)
        {
            ...
            if (!this.freeHeroList.Contains(heroId))
            {
                flag = false;
            }
            return flag;
        ...
        {
            uint skinCfgId = CSkinInfo.GetSkinCfgId(heroId, skinId);
            return (this.heroExperienceSkinDic.ContainsKey(skinCfgId) && (Singleton<CRoleInfoManager>.GetInstance().GetMasterRoleInfo().getCurrentTimeSinceLogin() < ((ulong) this.heroExperienceSkinDic[skinCfgId])));
        }
        ...
        private void OnGlobalRefreshTimerEnd(int seq)
        {
            ...
            if (masterRoleInfo != null)
            {
                ...
                this.SetGlobalRefreshTimer((uint) num, false);
            }
        }
        ...
        {
            uint num = Utility.GetNewDayDeltaSec(this.getCurrentTimeSinceLogin()) * 0x3e8;
        ...
        public void OnUpdate(ref SCDT_NTF_ACNT_INFO_UPD ntfAcntInfoUpd)
        {
            switch (ntfAcntInfoUpd.bUpdType)
            {
                ...
                case 15:
                    ...
                    {
                        Singleton<CUIManager>.GetInstance().OpenTips(string.Format("经验池增加{0}", ntfAcntInfoUpd.iUpdValChg), false, 1f, null, new object[0]);
                    }
                    break;
        ...
        public void ReqSetInBattleNewbieBit(uint id, bool bIsLastAge)
        {
            ...
            if (Singleton<BattleLogic>.GetInstance().isRuning && ((curLvelContext != null) && (curLvelContext.GameType == COM_GAME_TYPE.COM_SINGLE_GAME_OF_GUIDE)))
            {
                stInBattleLevelBits bits;
                if (this.inBattleNewbieBits.TryGetValue(curLvelContext.iLevelID, out bits))
                {
                ...
                msg.stPkgData.stDyeInBattleNewbieBitReq.iLevelID = curLvelContext.iLevelID;
                ...
                this.SetInBattleNewbieBit(curLvelContext.iLevelID, id, bIsLastAge);
            }
        }
        ...
        public void SetAttributes(SCPKG_CMD_GAMELOGINRSP rsp)
        {
            ...
            this.m_gradeOfRank = rsp.bGradeOfRank;
            this.m_classOfRank = rsp.dwClassOfRank;
            ...
            if (Utility.ToUtcTime2Local((long) rsp.dwServerCurTimeSec).get_Date() == time.get_Date())
            {
        ...
        public void SetFreeHeroInfo(COMDT_FREEHERO stFreeHero)
        {
            ...
            for (int i = 0; i < stFreeHero.stFreeHeroList.wFreeCnt; i++)
            {
                ...
                {
                    ResHeroCfgInfo dataByKey = GameDataMgr.heroDatabin.GetDataByKey(stFreeHero.stFreeHeroList.FreeHeroID[i]);
                    if ((dataByKey != null) && (dataByKey.bIOSHide == 1))
                    {
                        continue;
                    }
                }
                this.freeHeroList.Add(stFreeHero.stFreeHeroList.FreeHeroID[i]);
            }
            if ((this.freeHeroExpireTime != 0) && (freeHeroExpireTime != this.freeHeroExpireTime))
            {
                ...
                int num3 = (int) ((this.freeHeroExpireTime - this.getCurrentTimeSinceLogin()) + ((ulong) 15L));
        ...
        {
            this.m_sysTime = serverTime;
            this.m_upToLoginSec = Time.realtimeSinceStartup;
        }

        private void setShopBuyRcd(ref CSDT_ACNT_SHOPBUY_INFO rcd)
        {
            this.SetServerTime(rcd.iGameSysTime);
            this.m_freeDrawInfo = new stShopBuyDrawInfo[rcd.astShopDrawInfo.Length];
            for (int i = 0; i < this.m_freeDrawInfo.Length; i++)
            {
                this.m_freeDrawInfo[i].dwLeftFreeDrawCD = this.m_sysTime + rcd.astShopDrawInfo[i].iLeftFreeSec;
            ...
            if (rcd.stSymbolDrawSenior != null)
            {
                ...
                {
                    for (byte j = 0; j < rcd.stSymbolDrawSenior.bSubDrawCnt; j = (byte) (j + 1))
                    {
                        COMDT_DRAWCNT_SUBINFO comdt_drawcnt_subinfo = rcd.stSymbolDrawSenior.astSubDrawInfo[j];
                        if (comdt_drawcnt_subinfo != null)
                        {
                            switch (comdt_drawcnt_subinfo.iSubType)
                            {
                                case 2:
                                    CMallSymbolGiftController.SymbolGiftSeniorDrawedCntInfo[comdt_drawcnt_subinfo.iSubType] = comdt_drawcnt_subinfo.iDrawCnt;
                                    break;
                ...
                int iFreeDrawTotalCnt = rcd.stSymbolDrawSenior.iFreeDrawTotalCnt;
                if (iFreeDrawTotalCnt >= 0)
                {
                    CMallSymbolGiftController.SymbolGiftSeniorDrawedCntInfo[1] = iFreeDrawTotalCnt;
                }
            }
        }
        ...
        public uint PvpNeedExp
        {
            get
            {
                ResAcntPvpExpInfo dataByKey = GameDataMgr.acntPvpExpDatabin.GetDataByKey((byte) this.m_pvpLevel);
                if (dataByKey != null)
                {
