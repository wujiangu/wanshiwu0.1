namespace Assets.Scripts.UI
{
    ...
    public class CUIManager : Singleton<CUIManager>
    {
        ...
        {
            string[] exceptFormNames = new string[] { LobbyForm.FORM_PATH, SysEntryForm.FORM_PATH, CChatController.ChatFormPath };
        ...
        public override void Init()
        {
            ...
            Singleton<CUIEventManager>.GetInstance().AddUIEventListener(enUIEventID.UI_OnFormHide, new CUIEventManager.OnUIEventHandler(this.OnFormHide));
        }
        ...
        public void OpenAwardTip(CUseable[] items, string title = null, bool playSound = false, enUIEventID eventID = 0, bool displayAll = false, bool forceNotGoToBag = false, string formPath = "Form_Award")
        {
            ...
            {
                int b = 5;
                int num2 = Mathf.Min(items.Length, b);
                ...
                if (formScript != null)
                {
                    ...
                    {
                        int num5 = i + 1;
                        GameObject gameObject = formScript.transform.FindChild("IconContainer/itemCell" + num5.ToString()).gameObject;
                    ...
                    for (int j = num2; j < b; j++)
                    {
                        int num6 = j + 1;
                        formScript.transform.FindChild("IconContainer/itemCell" + num6.ToString()).gameObject.CustomSetActive(false);
                    }
                    CUIEventScript component = formScript.transform.Find("btnGroup/Button_Use").GetComponent<CUIEventScript>();
                    component.gameObject.CustomSetActive(false);
                    if ((!forceNotGoToBag && (num2 == 1)) && (items[0].m_type == COM_ITEM_TYPE.COM_OBJTYPE_ITEMPROP))
                    {
                        CItem item = items[0] as CItem;
                        if ((item.m_itemData.bType == 4) || (item.m_itemData.bType == 1))
                        {
                            ...
                            if (useableByBaseID != null)
                            {
                                ...
                                component.m_onClickEventParams.iconUseable = useableByBaseID;
                                component.m_onClickEventParams.tag = Mathf.Min(item.m_stackCount, useableByBaseID.m_stackCount);
                            }
                        }
                    }
                }
        ...
        {
            ResRuleText dataByKey = GameDataMgr.s_ruleTextDatabin.GetDataByKey(txtKey);
            if (dataByKey != null)
            {
        ...
        public void OpenTips(string strContent, bool bReadDatabin = false, float timeDuration = 1f, GameObject referenceGameObject = null, params object[] replaceArr)
        {
        ...
        private void UpdateFormHided()
        {
            ...
            {
                if (!this.m_forms[i].IsHided() || this.m_forms[i].IsHidedByOtherForm())
                {
                    if (flag)
                    {
                        this.m_forms[i].HideByOtherForm();
                    }
                    else if (this.m_forms[i].IsHidedByOtherForm())
                    {
                        this.m_forms[i].RevertToVisibleFromHideByOtherForm();
                    }
                    if (!flag && this.m_forms[i].m_hideUnderForms)
                    {
                        flag = true;
                    }
                }
