namespace Assets.Scripts.GameLogic
{
    ...
    public class HurtComponent : LogicComponent
    {
        ...
        {
            int num = 0;
            num = (((hurt.hurtValue + ((hurt.adValue * hurt.attackInfo.iActorATT) / 0x2710)) + ((hurt.apValue * hurt.attackInfo.iActorINT) / 0x2710)) + ((hurt.hpValue * hurt.attackInfo.iActorMaxHp) / 0x2710)) * this.GetExtraHurtValue(ref hurt);
        ...
        private int CriticalDamagePart(ref HurtDataInfo hurt)
        {
            bool flag = false;
            int num = 0;
            if (hurt.atkSlot == SkillSlotType.SLOT_SKILL_0)
            {
        ...
        private int GetExtraHurtValue(ref HurtDataInfo hurt)
        {
            ...
            int num4 = 1 + (((hurt.loseHpValue * (totalValue - actorHp)) * 100) / 0x2710);
            switch (hurt.extraHurtType)
            {
                ...
                case ExtraHurtTypeDef.ExtraHurt_MaxHp:
                    return ((totalValue * num4) / 100);

                case ExtraHurtTypeDef.ExtraHurt_CurHp:
                    return (((totalValue - actorHp) * num4) / 100);

                case ExtraHurtTypeDef.ExtraHurt_LoseHp:
                    return ((actorHp * num4) / 100);
            }
            return 1;
        }
