namespace Assets.Scripts.GameLogic
{
    ...
    {
        private bool[] m_buttonHidden = new bool[7];
        private bool[] m_buttonHighLighted = new bool[7];
        ...
        public void Activate(SkillSlotType inSlotType, bool bActivate, bool bAll)
        {
            ...
            if ((!bActivate && (Singleton<CBattleSystem>.GetInstance().m_FormScript != null)) && (Singleton<CBattleSystem>.GetInstance().m_skillButtonManager != null))
            {
                if (bAll)
                {
                    for (int j = 0; j < 7; j++)
                    {
                        Singleton<CBattleSystem>.GetInstance().m_skillButtonManager.SkillButtonUp(Singleton<CBattleSystem>.GetInstance().m_FormScript, (SkillSlotType) j, false);
                    }
                }
                else
                {
                    Singleton<CBattleSystem>.GetInstance().m_skillButtonManager.SkillButtonUp(Singleton<CBattleSystem>.GetInstance().m_FormScript, inSlotType, false);
                }
            }
        }
        ...
        public void Highlight(SkillSlotType inSlotType, bool bHighlight, bool bAll, bool bDoActivating, bool bPauseGame)
        {
            ...
            for (int i = 0; i < list.Count; i++)
            {
                ...
                if (button != null)
                {
                    Singleton<CBattleSystem>.GetInstance().SetButtonHighLight(button, bHighlight);
                }
            }
                ...
                if (list.Count != 0)
                {
                    ...
                    for (int i = 0; i < list.Count; i++)
                    {
                        if (list[i] != null)
                        {
                            ...
                            if (transform != null)
                            {
                                Singleton<CBattleSystem>.GetInstance().SetLearnBtnHighLight(transform.gameObject, bHighlight);
                            }
                        }
                    }
                    if (inSlotType < SkillSlotType.SLOT_SKILL_COUNT)
        ...
        private void onResetSkillButtonManager()
        {
            for (int i = 0; i < 7; i++)
            {
                ...
                {
                    Singleton<CBattleSystem>.GetInstance().GetButton((SkillSlotType) i).m_button.CustomSetActive(false);
                }
            }
        }
        ...
        private void onSkillLearnBtnStateChange(int inSlotType, bool bShow)
        {
            ...
            {
                GameObject learnSkillButton = Singleton<CBattleSystem>.GetInstance().GetButton((SkillSlotType) inSlotType).GetLearnSkillButton();
        ...
        public List<GameObject> QuerySkillButtons(SkillSlotType inSlotType, bool bAll)
        {
            ...
            if (bAll)
            {
                for (int i = 0; i < 7; i++)
                {
                    list.Add(Singleton<CBattleSystem>.GetInstance().GetButton((SkillSlotType) i).m_button);
                }
                return list;
            }
            if (inSlotType < SkillSlotType.SLOT_SKILL_COUNT)
            {
                list.Add(Singleton<CBattleSystem>.GetInstance().GetButton(inSlotType).m_button);
            }
            return list;
        }

        public void Show(SkillSlotType inSlotType, bool bShow, bool bAll)
        {
            List<GameObject> list = this.QuerySkillButtons(inSlotType, bAll);
            for (int i = 0; i < list.Count; i++)
            {
            ...
            if ((!bShow && (Singleton<CBattleSystem>.GetInstance().m_FormScript != null)) && (Singleton<CBattleSystem>.GetInstance().m_skillButtonManager != null))
            {
                if (bAll)
                {
                    for (int j = 0; j < 7; j++)
