namespace Assets.Scripts.GameLogic
{
    ...
    public class TargetSearcher : Singleton<TargetSearcher>
    {
        ...
        public ActorRoot GetLowestHpTarget(ActorRoot InActor, int srchR, TargetPriority priotity, uint filter, bool bEnemy = true)
        {
                    ...
                    if (i != InActor.TheActorMeta.ActorCamp)
                    {
                        ...
                        for (int j = 0; j < num2; j++)
                        {
                            ...
                            if ((((filter & (((int) 1) << target.TheActorMeta.ActorType)) <= 0L) && target.HorizonMarker.IsVisibleFor(InActor.TheActorMeta.ActorCamp)) && InActor.CanAttack(target))
                            {
                                ...
                                else if (TypeSearchCondition.Fit(target, ActorTypeDef.Actor_Type_Monster) && DistanceSearchCondition.Fit(target, InActor, srchR))
                                {
        ...
        public ActorRoot GetNearestEnemy(ActorRoot InActor, int srchR, uint filter)
        {
            ActorRoot root = null;
            ulong num = (ulong) (srchR * srchR);
            for (int i = 0; i < 3; i++)
