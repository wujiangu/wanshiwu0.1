namespace Assets.Scripts.GameLogic
{
    ...
    public class GlobalTrigger : MonoBehaviour, ITrigger
    {
        ...
        private void onActorDead(ref DefaultGameEventParam prm)
        {
            ...
            {
                CTriggerMatch match = this.TriggerMatches[i];
                if ((match != null) && this.FilterMatch(EGlobalGameEvent.ActorDead, match, prm.src, prm.orignalAtker, (DefaultGameEventParam) prm, i))
                {
                    this.DoTriggering(match, prm.src, prm.orignalAtker, (DefaultGameEventParam) prm);
                }
            }
        }
        ...
        private void OnDestroy()
        {
            ...
            if (Singleton<GameSkillEventSys>.instance != null)
            {
                Singleton<GameSkillEventSys>.instance.RmvEventHandler<ActorSkillEventParam>(GameSkillEventDef.Event_UseSkill, new GameSkillEvent<ActorSkillEventParam>(this.onUseSkill));
        ...
        private void onEnterCombat(ref DefaultGameEventParam prm)
        {
            ...
            {
                CTriggerMatch match = this.TriggerMatches[i];
                if ((match != null) && this.FilterMatch(EGlobalGameEvent.EnterCombat, match, prm.src, prm.atker, (DefaultGameEventParam) prm, i))
                {
        ...
        private void onFightPrepare(ref DefaultGameEventParam prm)
        {
            ...
            {
                CTriggerMatch match = this.TriggerMatches[i];
                if ((match != null) && this.FilterMatch(EGlobalGameEvent.FightPrepare, match, prm.src, prm.atker, (DefaultGameEventParam) prm, i))
                {
                    this.DoTriggering(match, prm.src, prm.atker, (DefaultGameEventParam) prm);
                }
            }
