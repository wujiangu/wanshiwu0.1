namespace Assets.Scripts.GameLogic
{
    ...
    public class AchievementRecorder
    {
        ...
        public void Clear()
        {
            ...
            Singleton<GameEventSys>.instance.RmvEventHandler<DefaultGameEventParam>(GameEventDef.Event_ActorDead, new RefAction<DefaultGameEventParam>(this.OnActorDeath));
        }

        private void OnActorDeath(ref DefaultGameEventParam prm)
        {
            if ((prm.src != 0) && (prm.orignalAtker != 0))
            {
                bool flag = false;
        ...
        {
            this.Clear();
            Singleton<GameEventSys>.instance.AddEventHandler<DefaultGameEventParam>(GameEventDef.Event_ActorDead, new RefAction<DefaultGameEventParam>(this.OnActorDeath));
        }
    }
}
