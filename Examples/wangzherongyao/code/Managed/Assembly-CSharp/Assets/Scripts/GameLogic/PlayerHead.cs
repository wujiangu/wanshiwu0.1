namespace Assets.Scripts.GameLogic
{
    ...
    public class PlayerHead : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            bool bActive = Singleton<BattleLogic>.GetInstance().m_GameInfo.gameContext.IsSoulGrow();
        ...
        public void Init(HeroHeadHud hudOwner, PoolObjHandle<ActorRoot> myHero)
        {
            if (myHero != 0)
            {
                ...
                this.HeroHeadImg.SetSprite(CUIUtility.s_Sprite_Dynamic_BustCircle_Dir + CSkinInfo.GetHeroSkinPic(configId, 0), Singleton<CBattleSystem>.GetInstance().m_FormScript, true, false, false);
        ...
        public void OnMyHeroDead()
        {
            if (this._myHero != 0)
            {
                ...
                if ((Singleton<BattleLogic>.instance.GetCurLvelContext().GameType != COM_GAME_TYPE.COM_SINGLE_GAME_OF_BURNING) && (Singleton<BattleLogic>.instance.GetCurLvelContext().GameType != COM_GAME_TYPE.COM_SINGLE_GAME_OF_ARENA))
                {
        ...
        private void UpdateReviveCd()
        {
            if ((this.MyHero != 0) && (this.MyHero.handle.ActorControl != null))
            {
                ...
                else if (!Singleton<BattleLogic>.instance.GetCurLvelContext().isPVPMode)
                {
