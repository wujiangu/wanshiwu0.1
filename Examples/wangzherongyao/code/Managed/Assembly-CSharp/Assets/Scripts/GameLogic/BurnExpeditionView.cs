namespace Assets.Scripts.GameLogic
{
    ...
    public class BurnExpeditionView
    {
        ...
        private void Set_Award(GameObject container, int index, string icon, string count, string desc, CUIFormScript formScript)
        {
            ...
            {
                GameObject obj2 = Utility.FindChild(container, string.Format("itemCell{0}", index));
                obj2.CustomSetActive(true);
                Utility.GetComponetInChild<Image>(obj2, "imgIcon").SetSprite(icon, formScript, true, false, false);
                Utility.GetComponetInChild<Text>(obj2, "lblIconCount").gameObject.CustomSetActive(!string.IsNullOrEmpty(count));
                Utility.GetComponetInChild<Text>(obj2, "lblIconCount").text = count;
                if (!string.IsNullOrEmpty(desc))
                {
                    Utility.GetComponetInChild<Text>(obj2, "ItemName").text = desc;
                }
            }
        }
