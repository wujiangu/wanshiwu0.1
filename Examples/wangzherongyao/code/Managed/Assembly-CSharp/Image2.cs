...
public class Image2 : Image
{
    ...
    private void GenerateTiledSprite(List<UIVertex> vbo)
    {
        ...
        Vector2 vector7 = new Vector2(innerUV.z, innerUV.w);
        UIVertex simpleVert = UIVertex.simpleVert;
        simpleVert.color = base.color;
        ...
        if (base.fillCenter)
        {
            for (float i = y; i < num6; i += num2)
            {
                ...
                if (num8 > num6)
                {
                    uvMax.y = uvMin.y + (((vector7.y - uvMin.y) * (num6 - i)) / (num8 - i));
                    num8 = num6;
                }
                uvMax.x = vector7.x;
                for (float j = x; j < num4; j += num)
                {
                    ...
                    if (num10 > num4)
                    {
                        uvMax.x = uvMin.x + (((vector7.x - uvMin.x) * (num4 - j)) / (num10 - j));
                        num10 = num4;
                    }
        ...
        if (base.hasBorder)
        {
            ...
            for (float k = y; k < num6; k += num2)
            {
                ...
                if (num12 > num6)
                {
                    vector9.y = uvMin.y + (((vector7.y - uvMin.y) * (num6 - k)) / (num12 - k));
                    num12 = num6;
                }
                this.AddQuad(vbo, simpleVert, new Vector2(0f, k) + pixelAdjustedRect.position, new Vector2(x, num12) + pixelAdjustedRect.position, new Vector2(zero.x, uvMin.y), new Vector2(uvMin.x, vector9.y), vector5);
                this.AddQuad(vbo, simpleVert, new Vector2(num4, k) + pixelAdjustedRect.position, new Vector2(pixelAdjustedRect.width, num12) + pixelAdjustedRect.position, new Vector2(vector7.x, uvMin.y), new Vector2(zero.z, vector9.y), vector5);
            }
            vector9 = vector7;
            for (float m = x; m < num4; m += num)
            {
                ...
                if (num14 > num4)
                {
                    vector9.x = uvMin.x + (((vector7.x - uvMin.x) * (num4 - m)) / (num14 - m));
                    num14 = num4;
                }
                this.AddQuad(vbo, simpleVert, new Vector2(m, 0f) + pixelAdjustedRect.position, new Vector2(num14, y) + pixelAdjustedRect.position, new Vector2(uvMin.x, zero.y), new Vector2(vector9.x, uvMin.y), vector5);
                this.AddQuad(vbo, simpleVert, new Vector2(m, num6) + pixelAdjustedRect.position, new Vector2(num14, pixelAdjustedRect.height) + pixelAdjustedRect.position, new Vector2(uvMin.x, vector7.y), new Vector2(vector9.x, zero.w), vector5);
            }
            ...
            this.AddQuad(vbo, simpleVert, new Vector2(num4, 0f) + pixelAdjustedRect.position, new Vector2(pixelAdjustedRect.width, y) + pixelAdjustedRect.position, new Vector2(vector7.x, zero.y), new Vector2(zero.z, uvMin.y), vector5);
            this.AddQuad(vbo, simpleVert, new Vector2(0f, num6) + pixelAdjustedRect.position, new Vector2(x, pixelAdjustedRect.height) + pixelAdjustedRect.position, new Vector2(zero.x, vector7.y), new Vector2(uvMin.x, zero.w), vector5);
            this.AddQuad(vbo, simpleVert, new Vector2(num4, num6) + pixelAdjustedRect.position, new Vector2(pixelAdjustedRect.width, pixelAdjustedRect.height) + pixelAdjustedRect.position, new Vector2(vector7.x, vector7.y), new Vector2(zero.z, zero.w), vector5);
        }
    }
    ...
    {
        if ((base.overrideSprite == null) || (this.alphaTexLayout == ImageAlphaTexLayout.None))
        {
            base.OnFillVBO(vbo);
        }
        else
