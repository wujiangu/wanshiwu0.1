namespace AGE
{
    ...
    public class Action : PooledClassObject
    {
        ...
        public uint updateFrameIdx;
        public static readonly uint updateInterval = 1;
        ...
        {
            this.updateFrameIdx = (Singleton<FrameSynchr>.GetInstance().CurFrameNum + 1) % updateInterval;
        ...
        {
            if ((ActionManager.Instance.frameMode && this.enabled) && (!this.playSpeed.IsZero && (this.updateFrameIdx == (Singleton<FrameSynchr>.instance.CurFrameNum % updateInterval))))
            {
                if (!this.started_)
                {
                    this.ForceStart();
