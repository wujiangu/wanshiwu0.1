    ...
    public class ActionManager : MonoSingleton<ActionManager>
    {
        ...
        private Action InternalPlayAction(string _actionName, bool _autoPlay, bool _stopConflictAction, GameObject[] _gameObjects)
        {
            ...
            if ((obj2 != null) && _stopConflictAction)
            {
                ...
                ListView<Action>.Enumerator enumerator = this.objectReferenceSet[obj2].GetEnumerator();
                while (enumerator.MoveNext())
                {
                ...
                ListView<Action>.Enumerator enumerator2 = view.GetEnumerator();
                while (enumerator2.MoveNext())
                {
                    enumerator2.Current.Stop(false);
                }
            }
            Action action3 = this.LoadActionResource(_actionName);
            if (action3 == null)
            {
                Debug.LogError("Playing \"" + _actionName + "\" failed. Asset not found!");
                return null;
            }
            Action item = ClassObjPool<Action>.Get();
            ...
            for (int j = 0; j < _gameObjects.Length; j++)
            {
                ...
                if (obj4 != null)
                {
                    ListView<Action> view3 = null;
                    if (this.objectReferenceSet.TryGetValue(obj4, out view3))
                    {
                        view3.Add(item);
                    }
                    else
                    {
                        view3 = new ListView<Action>();
                        view3.Add(item);
                        this.objectReferenceSet.Add(obj4, view3);
                    }
                }
            }
            return item;
            ...
            if (_action != null)
            {
                ...
                for (int i = 0; i < count; i++)
                {
                    ...
                    if (key != null)
                    {
                        ...
                        {
                            ListView<Action> view2 = this.objectReferenceSet[key];
        ...
        public void StopAllActions()
        {
            ...
            {
                ListView<Action> view = new ListView<Action>(this.actionList);
                for (int i = 0; i < view.Count; i++)
                {
                    view[i].Stop(true);
                }
        ...
        public void UpdateLogic(int nDelta)
        {
            if (this.frameMode)
            {
                for (int i = 0; i < this.waitReleaseList.Count; i++)
                {
                    ...
                    {
                        PoolObjHandle<Action> handle = this.waitReleaseList[i];
                ...
                for (int m = 0; m < this.waitReleaseList.Count; m++)
                {
                    ...
                    {
                        PoolObjHandle<Action> handle2 = this.waitReleaseList[m];
