...
public class ApolloVoice_lib : IApolloVoice
{
    ...
    public int _CloseMic()
    {
        ...
        if (mApolloVoice == null)
        {
            mApolloVoice = new AndroidJavaClass("com.tencent.apollo.ApolloVoiceDeviceMgr");
        }
        if (mApolloVoice == null)
        {
            Debug.Log("Unity Find mApolloVoice Failed");
            return -1;
        }
        Debug.Log("Unity ApolloVoiceDeviceExitVoipMode");
        mApolloVoice.CallStatic("ApolloVoiceDeviceExitVoipMode", new object[0]);
    ...
    public int _GetMemberState(int[] memberState)
    {
        ...
        {
            char ch = str2[i];
            if ((ch.ToString() == "#") || (i == (str2.Length - 1)))
            {
                if (i == (str2.Length - 1))
                {
                    num4 = str2.Length;
                ...
                int.TryParse(str2.Substring(startIndex, num4 - startIndex), out memberState[index]);
                startIndex = i + 1;
                index++;
            }
        }
    ...
    public int _OpenMic()
    {
        ...
        Debug.Log("ApolloVoiceC# API: ApolloVoiceDeviceEnterVoipMode");
        int num = ApolloVoiceGetPhoneMode();
        object[] args = new object[] { num };
        mApolloVoice.CallStatic("ApolloVoiceDeviceEnterVoipMode", args);
        int num2 = ApolloVoiceOpenMic();
        if (num2 == 0)
        {
            return 0;
        }
        Debug.Log("ApolloVoiceC# API: _OpenMic Failed nRet=" + num2);
        return num2;
    }
