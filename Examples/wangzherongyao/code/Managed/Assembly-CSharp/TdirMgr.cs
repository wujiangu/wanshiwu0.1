...
public class TdirMgr : MonoSingleton<TdirMgr>
{
    ...
    private static bool mIsIOSCheckVersion;
    ...
    {
        return new <QueryTdirAsync>c__Iterator1D { info = info, failCallBack = failCallBack, reasync = reasync, successCallBack = successCallBack, <$>info = info, <$>failCallBack = failCallBack, <$>reasync = reasync, <$>successCallBack = successCallBack, <>f__this = this };
    }
    ...
    private static int InternalNodeID
    {
        get
        {
            return (!mIsIOSCheckVersion ? 1 : 1);
        }
    }

    public static bool IsIOSCheckVersion
    {
        get
        {
            return mIsIOSCheckVersion;
        }
    }
    ...
    private static int WeixinNodeID
    {
        get
        {
            ...
            return (!mIsIOSCheckVersion ? 3 : 6);
        }
    }

    [CompilerGenerated]
    private sealed class <QueryTdirAsync>c__Iterator1D : IDisposable, IEnumerator, IEnumerator<object>
    {
        ...
        internal List<KeyValuePair<string, string>> <events>__5;
        ...
        Label_03BF:
            if (this.<>f__this.mAsyncFinished != AsyncStatu.AsyncSuccess)
            {
                ...
                this.<events>__4.Add(new KeyValuePair<string, string>("status", "0"));
                ...
                this.<events>__4.Add(new KeyValuePair<string, string>("errorcode", this.<>f__this.m_GetinnerResult.ToString()));
                Singleton<ApolloHelper>.GetInstance().ApolloRepoertEvent("Service_TDirLoad", this.<events>__4, true);
            }
            else
            {
                TdirMgr.mIsIOSCheckVersion = false;
                ...
                if (!TdirMgr.IsIOSCheckVersion)
                {
                }
                this.<events>__5 = new List<KeyValuePair<string, string>>();
                this.<events>__5.Add(new KeyValuePair<string, string>("WorldID", MonoSingleton<TdirMgr>.GetInstance().SelectedTdir.logicWorldID.ToString()));
                this.<events>__5.Add(new KeyValuePair<string, string>("status", "1"));
                float num3 = Time.time - this.<>f__this.m_GetTdirTime;
                this.<events>__5.Add(new KeyValuePair<string, string>("totaltime", num3.ToString()));
                this.<events>__5.Add(new KeyValuePair<string, string>("errorcode", this.<>f__this.m_GetinnerResult.ToString()));
                Singleton<ApolloHelper>.GetInstance().ApolloRepoertEvent("Service_TDirLoad", this.<events>__5, true);
