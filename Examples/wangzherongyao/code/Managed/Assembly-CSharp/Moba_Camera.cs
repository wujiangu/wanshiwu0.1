    ...
    private void CalculateCameraMovement()
    {
        ...
        if ((this.requirements != null) && (this.requirements.pivot != null))
        {
            ...
            else
            {
                ...
                Vector3 from = new Vector3(0f, this.requirements.pivot.position.y, 0f);
                ...
                Vector3 vector17 = zero - from;
                if (vector17.magnitude > 0.001f)
                {
                    Vector3 vector6 = Vector3.Lerp(from, zero, this.settings.movement.lockTransitionRate);
