namespace Pathfinding.RVO
{
    ...
    public class RVONavmesh : GraphModifier
    {
        ...
        {
            <AddGraphObstacles>c__AnonStorey25 storey = new <AddGraphObstacles>c__AnonStorey25 {
                sim = sim,
                <>f__this = this
            };
            if (((this.obstacles.Count > 0) && (this.lastSim != null)) && (this.lastSim != storey.sim))
            {
                this.RemoveObstacles();
            }
            this.lastSim = storey.sim;
            INavmesh navmesh = graph as INavmesh;
            if (navmesh != null)
            {
                storey.uses = new int[20];
                navmesh.GetNodes(new GraphNodeDelegateCancelable(storey.<>m__0));
            }
        }
        ...
        [CompilerGenerated]
        private sealed class <AddGraphObstacles>c__AnonStorey25
        {
            internal RVONavmesh <>f__this;
            internal Simulator sim;
            internal int[] uses;

            internal bool <>m__0(GraphNode _node)
            {
                int num6;
