...
public class CRoleRegisterSys : Singleton<CRoleRegisterSys>
{
    ...
    public string GetRandomName()
    {
        ...
        return string.Format("{0}{1}", Utility.UTF8Convert(dataByKey.szName), Utility.UTF8Convert(ea.szName));
    }
    ...
    private void OnRoleCreate(CUIEvent cuiEvent)
    {
        ...
        switch (Utility.CheckRoleName(roleName))
        {
            ...
            case Utility.NameResult.Null:
                Singleton<CUIManager>.instance.OpenTips("RoleRegister_InputName", true, 1f, null, new object[0]);
                break;

            case Utility.NameResult.OutOfLength:
                Singleton<CUIManager>.instance.OpenTips("RoleRegister_NameOutOfLength", true, 1f, null, new object[0]);
                break;

            case Utility.NameResult.InVaildChar:
                Singleton<CUIManager>.instance.OpenTips("RoleRegister_NameInvaildChar", true, 1f, null, new object[0]);
                break;
        }
    }
    ...
    public void ShowErrorCode(int errorCode)
    {
        if (errorCode == 6)
        {
            Singleton<CUIManager>.GetInstance().OpenTips(Singleton<CTextManager>.GetInstance().GetText("RoleRegister_NameExists"), false, 1f, null, new object[0]);
        }
    }
