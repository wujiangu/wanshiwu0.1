...
public class PreloadActionAssets
{
    ...
    private void AddActionsFromSkill(DictionaryView<string, AGE.Action> actions, int skillID)
    {
        if (skillID > 0)
        {
            ...
            if (dataByKey != null)
            {
                ...
                {
                    ResBulletCfgInfo info2 = GameDataMgr.bulletDatabin.GetDataByKey(dataByKey.iBulletID);
                    if (info2 != null)
                    {
    ...
    private void AddActionsFromSkillCombine(DictionaryView<string, AGE.Action> actions, int skillCombineID)
    {
        if (skillCombineID > 0)
        {
            ResSkillCombineCfgInfo dataByKey = GameDataMgr.skillCombineDatabin.GetDataByKey(skillCombineID);
            if (dataByKey != null)
            {
                this.AddAction(actions, dataByKey.szPrefab);
            }
    ...
    {
        return new <Exec>c__Iterator17 { <>f__this = this };
    }
    ...
    [CompilerGenerated]
    private sealed class <Exec>c__Iterator17 : IDisposable, IEnumerator, IEnumerator<object>
    {
