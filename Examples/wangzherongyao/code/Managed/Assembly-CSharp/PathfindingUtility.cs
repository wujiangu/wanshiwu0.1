...
public class PathfindingUtility
{
    ...
    public static VInt3 Move(ActorRoot actor, VInt3 delta, out VInt groundY)
    {
        if (actor.isMovable)
        {
            groundY = (actor == null) ? 0 : actor.groundY;
            return InternalMove(actor.location, delta, ref groundY, actor);
        }
        groundY = actor.groundY;
        return VInt3.zero;
    }

    public static VInt3 Move(ActorRoot actor, VInt3 delta, out VInt groundY, out bool collided)
    {
        VInt3 num = Move(actor, delta, out groundY);
    ...
    private static void MoveFromNode(TriangleMeshNode node, int startEdge, VInt3 srcLoc, VInt3 destLoc, out VInt3 result)
    {
        result = srcLoc;
        while (node != null)
        {
            ...
            if (node.IsVertex(srcLoc, out num))
            {
                ...
                else
                {
                    ...
                    for (int j = 0; j < nodeInfos.Count; j++)
                    {
                        ...
                        {
                            int num8;
                            VFactor cosineAngle = info2.GetCosineAngle(destLoc, out num8);
                            if (cosineAngle > factor)
                            {
                                factor = cosineAngle;
                                num6 = num8;
                                node2 = info2.node;
                    ...
                    if (node2 != null)
                    {
                        MoveAlongEdge(node2, num6, srcLoc, destLoc, out result);
                        break;
                    }
                }
            }
            ...
            if (edge == -1)
            {
                ...
                else
                {
                    ...
                    if (edge != -1)
                    {
                        MoveAlongEdge(node, edge, srcLoc, destLoc, out result);
                    }
                }
                break;
            }
    ...
    public static VInt3 MoveLerp(ActorRoot actor, VInt3 srcPos, VInt3 delta, out VInt groundY)
    {
        ...
        {
            groundY = (actor == null) ? 0 : actor.groundY;
            return InternalMove(srcPos, delta, ref groundY, actor);
        }
        groundY = actor.groundY;
        return VInt3.zero;
    }
    ...
    private struct TMNodeInfo
    {
        ...
        public VInt3 v1;
        public VInt3 v2;
        public VFactor GetCosineAngle(VInt3 dest, out int edgeIndex)
        {
