...
internal class CheatCommandCommonEntry
{
    ...
    [CheatCommandEntryMethod("游戏/命令延迟", true, false)]
    public static string ShowCommandPing()
    {
        Singleton<FrameWindow>.GetInstance().ToggleShowPingChart();
        return CheatCommandBase.Done;
    }

    [CheatCommandEntryMethod("游戏/显示抖动", true, false)]
    public static string ShowGameJitter(int InFlag)
    {
        Singleton<FrameSynchr>.GetInstance().SwitchJitterChart(InFlag);
        return CheatCommandBase.Done;
    }
    ...
    [CheatCommandEntryMethod("迷雾开关", true, false)]
    public static string SwitchFogOfWar()
    {
        FogOfWar.enable = !FogOfWar.enable;
        return CheatCommandBase.Done;
    }
