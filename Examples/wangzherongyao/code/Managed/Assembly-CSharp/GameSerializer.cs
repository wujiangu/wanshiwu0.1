...
public class GameSerializer
{
    ...
    private bool CheckRename(GameObject go)
    {
        for (int i = 0; i < go.transform.childCount; i++)
        {
            ...
            for (int j = 0; j < i; j++)
            {
                if (gameObject.name.Equals(go.transform.GetChild(j).name))
                {
                    s_saveFailStr = GetGameObjectPathName(gameObject) + " named duplicated";
                    Debug.Log("!!!Donnot use the same gameobject name in same dir, gameobj " + GetGameObjectPathName(gameObject) + " named duplicated");
                    return true;
                }
            }
        }
    ...
    private static void DeserializeObject(BinaryNode objInfoNode, GameObject go)
    {
        ...
        if (node2 != null)
        {
            ...
            if (component != null)
            {
                ...
                if (buffer4 != null)
                {
                    component.lightmapTilingOffset = UnityBasetypeSerializer.BytesToVector4(buffer4);
                }
            }
        }
    }
    ...
    private static bool IsMINeedExport(MemberInfo mi)
    {
        ...
        {
            FieldInfo info = (FieldInfo) mi;
            if (info.IsLiteral && !info.get_IsInitOnly())
            {
                return false;
            }
            if (info.IsStatic)
