namespace CSProtocol
{
    ...
    public class COMDT_TRANSACTION_CONTEXT_DETAIL : ProtocolObject
    {
        ...
        private void select_1_21(long selector)
        {
            long num = selector;
            if ((num >= 1L) && (num <= 0x15L))
            {
                switch (((int) (num - 1L)))
                {
                    ...
                    case 15:
                        if (!(this.dataObject is COMDT_TRANS_CONTEXT_OF_TRANSVISITORSVRDATA))
                        {
                            if (this.dataObject != null)
                            {
                                this.dataObject.Release();
                            }
                            this.dataObject = (COMDT_TRANS_CONTEXT_OF_TRANSVISITORSVRDATA) ProtocolObjectPool.Get(COMDT_TRANS_CONTEXT_OF_TRANSVISITORSVRDATA.CLASS_ID);
                        }
                        return;
        ...
        public COMDT_TRANS_CONTEXT_OF_TRANSVISITORSVRDATA stTransVisitorSvrData
        {
            get
            {
                return (this.dataObject as COMDT_TRANS_CONTEXT_OF_TRANSVISITORSVRDATA);
            }
            set
            {
                this.dataObject = value;
