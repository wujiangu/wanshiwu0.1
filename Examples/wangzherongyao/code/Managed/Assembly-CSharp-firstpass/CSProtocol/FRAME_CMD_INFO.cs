namespace CSProtocol
{
    ...
    public class FRAME_CMD_INFO : ProtocolObject
    {
        ...
        private void select_1_30(long selector)
        {
            long num = selector;
            if ((num >= 1L) && (num <= 30L))
            {
                switch (((int) (num - 1L)))
                {
                    ...
                    case 0x10:
                        if (!(this.dataObject is FRAMEPKG_CMD_Signal_Panel))
                        {
                            if (this.dataObject != null)
                            {
                                this.dataObject.Release();
                            }
                            this.dataObject = (FRAMEPKG_CMD_Signal_Panel) ProtocolObjectPool.Get(FRAMEPKG_CMD_Signal_Panel.CLASS_ID);
                        }
                        return;
        ...
        public FRAMEPKG_CMD_Signal_Panel stCmdPlayerSendSignal
        {
            get
            {
                return (this.dataObject as FRAMEPKG_CMD_Signal_Panel);
            }
            set
            {
                this.dataObject = value;
