...
public class CResourcePackerInfo
{
    ...
    {
        return new <ASyncLoadAssetBundle>c__Iterator10 { ifsExtractPath = ifsExtractPath, <$>ifsExtractPath = ifsExtractPath, <>f__this = this };
    }

    public void CheckAlreadyContainResourceFileName(string fullPath)
    {
        if (!this.IsAllowDuplicateNames())
        {
    ...
    Label_0112:
        if (this.m_assetBundle == null)
        {
            ...
            object[] args = new object[] { filePath, CVersion.GetVersion(), CVersion.GetBuildNumber(), CVersion.GetRevisonNumber(), Singleton<CResourceManager>.GetInstance().GetCachedResourceVersion(), str2 };
    ...
    [CompilerGenerated]
    private sealed class <ASyncLoadAssetBundle>c__Iterator10 : IDisposable, IEnumerator, IEnumerator<object>
    {
