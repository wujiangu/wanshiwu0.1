    ...
    public class ResRewardMatchLevelInfo : IUnpackable, tsf4g_csharp_interface
    {
        ...
        public TdrError.ErrorType load(ref TdrReadBuf srcBuf, uint cutVer)
        {
            ...
            if (type == TdrError.ErrorType.TDR_NO_ERROR)
            {
                ...
                int num2 = 0x20;
                if (this.szMusicStartEvent_ByteArray.GetLength(0) < num2)
                {
                    this.szMusicStartEvent_ByteArray = new byte[LENGTH_szMusicStartEvent];
                }
                type = srcBuf.readCString(ref this.szMusicStartEvent_ByteArray, num2);
                if (type != TdrError.ErrorType.TDR_NO_ERROR)
                {
                    return type;
                }
                int num3 = 0x20;
                if (this.szMusicEndEvent_ByteArray.GetLength(0) < num3)
                {
                    this.szMusicEndEvent_ByteArray = new byte[LENGTH_szMusicEndEvent];
                }
                type = srcBuf.readCString(ref this.szMusicEndEvent_ByteArray, num3);
                if (type != TdrError.ErrorType.TDR_NO_ERROR)
                {
                    return type;
                }
                int num4 = 0x20;
                if (this.szAmbientSoundEvent_ByteArray.GetLength(0) < num4)
                {
                    this.szAmbientSoundEvent_ByteArray = new byte[LENGTH_szAmbientSoundEvent];
                }
                type = srcBuf.readCString(ref this.szAmbientSoundEvent_ByteArray, num4);
                if (type != TdrError.ErrorType.TDR_NO_ERROR)
                {
                    return type;
                }
                int num5 = 0x20;
                if (this.szBankResourceName_ByteArray.GetLength(0) < num5)
                {
                    this.szBankResourceName_ByteArray = new byte[LENGTH_szBankResourceName];
                }
                type = srcBuf.readCString(ref this.szBankResourceName_ByteArray, num5);
                if (type != TdrError.ErrorType.TDR_NO_ERROR)
                {
                    return type;
                }
