...
public class AstarPath : MonoBehaviour
{
    ...
    private static void CalculatePathsThreaded(object _threadInfo)
    {
        ...
        try
        {
            ...
            long targetTick = DateTime.get_UtcNow().Ticks + num;
            while (true)
            {
                ...
                long ticks = DateTime.get_UtcNow().Ticks;
                long num4 = 0L;
                p.Prepare();
                if (!p.IsDone())
                {
                    ...
                    while (!p.IsDone())
                    {
                        ...
                        num4 += DateTime.get_UtcNow().Ticks - ticks;
                        Thread.Sleep(0);
                        ticks = DateTime.get_UtcNow().Ticks;
                ...
                if (DateTime.get_UtcNow().Ticks > targetTick)
                {
                    Thread.Sleep(1);
                    targetTick = DateTime.get_UtcNow().Ticks + num;
                }
            }
        }
    ...
    {
        <FloodFill>c__AnonStorey1A storeya = new <FloodFill>c__AnonStorey1A {
            <>f__this = this
        };
        ...
        if (this.astarData.graphs != null)
        {
            ...
            storeya.smallAreasDetected = 0;
            storeya.warnAboutAreas = false;
            storeya.smallAreaList = ListPool<GraphNode>.Claim();
            for (int j = 0; j < this.graphs.Length; j++)
            {
                NavGraph graph2 = this.graphs[j];
                if (graph2 != null)
                {
                    GraphNodeDelegateCancelable del = new GraphNodeDelegateCancelable(storeya.<>m__B);
                    graph2.GetNodes(del);
                }
            }
            this.lastUniqueAreaIndex = storeya.area;
            if (storeya.warnAboutAreas)
            {
            ...
            if (storeya.smallAreasDetected > 0)
            {
                AstarLog(string.Concat(new object[] { storeya.smallAreasDetected, " small areas were detected (fewer than ", this.minAreaSize, " nodes),these might have the same IDs as other areas, but it shouldn't affect pathfinding in any significant way (you might get All Nodes Searched as a reason for path failure).\nWhich areas are defined as 'small' is controlled by the 'Min Area Size' variable, it can be changed in the A* inspector-->Settings-->Min Area Size\nThe small areas will use the area id ", 0x1ffff }));
            }
            ListPool<GraphNode>.Release(storeya.smallAreaList);
        }
    }
    ...
    {
        <GetNearest>c__AnonStorey1E storeye = new <GetNearest>c__AnonStorey1E();
        if (this.graphs == null)
        {
            return null;
        }
        storeye.minDist = float.PositiveInfinity;
        storeye.nearestNode = null;
        storeye.lineDirection = ray.direction;
        storeye.lineOrigin = ray.origin;
        for (int i = 0; i < this.graphs.Length; i++)
        {
            this.graphs[i].GetNodes(new GraphNodeDelegateCancelable(storeye.<>m__F));
        }
        return storeye.nearestNode;
    }
    ...
    private int ProcessWorkItems(bool force)
    {
        ...
        while (this.workItems.Count > 0)
        {
            ...
            try
            {
                flag = (item.update != null) ? item.update.Invoke(force) : true;
            }
            catch
            {
    ...
    public void QueueGraphUpdates()
    {
        if (!this.isRegisteredForUpdate)
        {
            ...
            AstarWorkItem itm = new AstarWorkItem {
                ...
                update = new Func<bool, bool>(this, (IntPtr) this.ProcessGraphUpdates)
            };
    ...
    public void ReturnPaths(bool timeSlice)
    {
        ...
        long num = !timeSlice ? 0L : (DateTime.get_UtcNow().Ticks + 0x2710L);
        int num2 = 0;
        while (this.pathReturnPop != null)
        {
            ...
            {
                num2 = 0;
                if (DateTime.get_UtcNow().Ticks >= num)
                {
                    return;
                }
            }
    ...
    public void ScanLoop(OnScanStatus statusCallback)
    {
        ...
        if (this.graphs != null)
        {
            ...
            this.astarData.UpdateShortcuts();
            if (storeyc.statusCallback != null)
            {
                storeyc.statusCallback(new Progress(0.05f, "Pre processing graphs"));
            }
            if (OnPreScan != null)
            {
                OnPreScan(this);
            ...
            DateTime time = DateTime.get_UtcNow();
            for (int i = 0; i < this.graphs.Length; i++)
            {
            ...
            while (storeyd.i < this.graphs.Length)
            {
                <ScanLoop>c__AnonStorey1B storeyb = new <ScanLoop>c__AnonStorey1B {
                    <>f__ref$28 = storeyc,
                    <>f__ref$29 = storeyd
                };
                NavGraph graph = this.graphs[storeyd.i];
                if (graph == null)
                {
                    if (storeyc.statusCallback != null)
                    {
                        object[] objArray1 = new object[] { "Skipping graph ", storeyd.i + 1, " of ", this.graphs.Length, " because it is null" };
                        storeyc.statusCallback(new Progress(AstarMath.MapTo(0.05f, 0.7f, (storeyd.i + 0.5f) / ((float) (this.graphs.Length + 1))), string.Concat(objArray1)));
                    }
                }
                else
                {
                    if (OnGraphPreScan != null)
                    {
                        if (storeyc.statusCallback != null)
                        {
                            object[] objArray2 = new object[] { "Scanning graph ", storeyd.i + 1, " of ", this.graphs.Length, " - Pre processing" };
                            storeyc.statusCallback(new Progress(AstarMath.MapToRange(0.1f, 0.7f, ((float) storeyd.i) / ((float) this.graphs.Length)), string.Concat(objArray2)));
                        }
                        OnGraphPreScan(graph);
                    }
                    storeyb.minp = AstarMath.MapToRange(0.1f, 0.7f, ((float) storeyd.i) / ((float) this.graphs.Length));
                    storeyb.maxp = AstarMath.MapToRange(0.1f, 0.7f, (storeyd.i + 0.95f) / ((float) this.graphs.Length));
                    if (storeyc.statusCallback != null)
                    {
                        object[] objArray3 = new object[] { "Scanning graph ", storeyd.i + 1, " of ", this.graphs.Length };
                        storeyc.statusCallback(new Progress(storeyb.minp, string.Concat(objArray3)));
                    }
                    OnScanStatus status = null;
                    if (storeyc.statusCallback != null)
                    {
                        status = new OnScanStatus(storeyb.<>m__D);
                    }
                    graph.ScanInternal(status);
                    graph.GetNodes(new GraphNodeDelegateCancelable(storeyb.<>m__E));
                    if (OnGraphPostScan != null)
                    {
                        if (storeyc.statusCallback != null)
                        {
                            object[] objArray4 = new object[] { "Scanning graph ", storeyd.i + 1, " of ", this.graphs.Length, " - Post processing" };
                            storeyc.statusCallback(new Progress(AstarMath.MapToRange(0.1f, 0.7f, (storeyd.i + 0.95f) / ((float) this.graphs.Length)), string.Concat(objArray4)));
                        }
            ...
            if (storeyc.statusCallback != null)
            {
                storeyc.statusCallback(new Progress(0.8f, "Post processing graphs"));
            }
            if (OnPostScan != null)
            {
                OnPostScan(this);
            ...
            if (storeyc.statusCallback != null)
            {
                storeyc.statusCallback(new Progress(0.9f, "Computing areas"));
            }
            this.FloodFill();
            this.VerifyIntegrity();
            if (storeyc.statusCallback != null)
            {
                storeyc.statusCallback(new Progress(0.95f, "Late post processing"));
            }
            ...
            TimeSpan span = (TimeSpan) (DateTime.get_UtcNow() - time);
    ...
    private sealed class <CalculatePaths>c__Iterator6 : IDisposable, IEnumerator, IEnumerator<object>
    {
        ...
        public bool MoveNext()
        {
            ...
            switch (num)
            {
                case 0:
                    ...
                    this.<targetTick>__6 = DateTime.get_UtcNow().Ticks + this.<maxTicks>__5;
                    break;
        ...
        Label_0156:
            ...
            this.<numPaths>__2++;
            this.<startTicks>__9 = DateTime.get_UtcNow().Ticks;
            ...
            if (!this.<p>__7.IsDone())
            {
                ...
                while (!this.<p>__7.IsDone())
                {
                    ...
                    this.<totalTicks>__10 += DateTime.get_UtcNow().Ticks - this.<startTicks>__9;
                    this.$current = null;
                    this.$PC = 2;
                    goto Label_03ED;
                Label_0281:
            ...
            if (DateTime.get_UtcNow().Ticks <= this.<targetTick>__6)
            {
                goto Label_00E7;
            }
            this.$current = null;
    ...
    [CompilerGenerated]
    private sealed class <FloodFill>c__AnonStorey1A
    {
        internal AstarPath <>f__this;
        internal uint area;
    ...
    [CompilerGenerated]
    private sealed class <GetNearest>c__AnonStorey1E
    {
    ...
    [CompilerGenerated]
    private sealed class <ScanLoop>c__AnonStorey1B
    {
        internal AstarPath.<ScanLoop>c__AnonStorey1C <>f__ref$28;
        ...
        internal void <>m__D(Progress p)
        {
            ...
            this.<>f__ref$28.statusCallback(p);
        }

        internal bool <>m__E(GraphNode node)
        {
            node.GraphIndex = (uint) this.<>f__ref$29.i;
            return true;
        }
    }
