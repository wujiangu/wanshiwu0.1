...
public class ArgumentDescriptionEnum : IArgumentDescription
{
    ...
    {
        <FilteredCandinatesInner>c__AnonStorey3F storeyf = new <FilteredCandinatesInner>c__AnonStorey3F {
            InArgument = InArgument
        };
        List<string> candinates = this.GetCandinates(InType);
        if ((candinates != null) && (storeyf.InArgument != null))
        {
            candinates.RemoveAll(new Predicate<string>(storeyf.<>m__3F));
        }
        return candinates;
    }
    ...
    [CompilerGenerated]
    private sealed class <FilteredCandinatesInner>c__AnonStorey3F
    {
        internal string InArgument;

        internal bool <>m__3F(string x)
        {
